...
**Objective:** Verify `get_spw_info()` can query SPW metadata from Measurement
Sets.
...
**Note:** This is expected behavior - the pipeline correctly validates
calibration status.

### Test 3: Adaptive Binning Algorithm ✓ PASSED

**Objective:** Verify adaptive binning algorithm works correctly with simulated
data.
...
**Result:** Algorithm correctly identified multiple detections with optimal
binning.
...
**Issue:** WSClean doesn't directly support SPW selection via command-line
flags.

**Current Implementation:**

- SPW selection works with CASA tclean backend
- For WSClean, SPW selection must be handled via frequency range or separate MS
  files
...
1. **Find Calibrated MS:** Locate properly calibrated MS file for full
   end-to-end testing
...
- Test data: `/stage/dsa110-contimg/ms/2025-10-28T13:55:53.fast.ms`
  (uncalibrated)
...
The implementation is ready for production testing with properly calibrated
Measurement Sets.
