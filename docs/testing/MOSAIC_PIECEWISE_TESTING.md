...
The mosaic build process is complex with multiple interdependent stages. To enable efficient debugging, we've created a piecewise testing script that isolates each stage.
...
**Purpose:** Verify that WCS metadata can be extracted from FITS files via the cache.
...
**Note:** Full coordinate system creation requires complex CASA API usage and is tested in the full build.
...
2. **Fast Iteration:** Failed tests can be rerun quickly without rebuilding everything
3. **Clear Error Messages:** Failures are immediately localized to specific stages
...
Once piecewise tests pass, the full build should work, with any remaining issues likely in:
- PB image handling
- Weight calculation
