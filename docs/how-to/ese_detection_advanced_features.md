...
This guide covers the advanced features available for ESE (Extreme Scattering Event) detection, including multi-metric scoring, threshold presets, caching, parallel processing, and multi-frequency/multi-observable analysis.
...
Instead of manually specifying `min_sigma` values, you can use predefined threshold presets:
...
Composite scoring combines multiple variability metrics (sigma_deviation, chi2_nu, eta_metric) into a single confidence score.
...
Variability statistics are cached to improve performance for frequently accessed sources.
...
- **Caching**: Reduces database queries by ~50-80% for frequently accessed sources
...
- **Multi-frequency/Multi-observable**: Adds significant overhead, use selectively
...
5. Use **multi-frequency/multi-observable** analysis for high-priority candidates only
