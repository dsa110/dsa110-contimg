...
For error detection to work in non-interactive shells (agentic sessions), `BASH_ENV` must be:
...
The **most reliable** approach is to have agents explicitly source the setup script:
...
**Non-interactive shells source nothing by default** - they only source `BASH_ENV` if it's set.
...
But the **only** way to guarantee it works in **all** non-interactive shells is to ensure `BASH_ENV` is set in the environment before bash starts.
