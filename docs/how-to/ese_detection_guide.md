...
Extreme Scattering Events (ESE) are transient phenomena where radio sources show significant flux variability (>5σ). This guide covers how to use the ESE detection system in the DSA-110 pipeline.
...
- `--products-db PATH`: Path to products database (default: `state/products.sqlite3`)
...
ESE detection is automatically triggered after photometry measurements when enabled.
...
- `PIPELINE_PRODUCTS_DB`: Path to products database (default: `state/products.sqlite3`)
...
- Verify variability stats are computed: `SELECT COUNT(*) FROM variability_stats`
