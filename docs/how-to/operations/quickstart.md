...
- API at `http://localhost:${CONTIMG_API_PORT}/api/status` (dashboard at `/ui`,
  QA at `/api/qa`)
...
- The pipeline stamps `UVData.telescope_name` and
  `MS::OBSERVATION.TELESCOPE_NAME` with `PIPELINE_TELESCOPE_NAME` (default
  `DSA_110`).
- Coordinates used (OVRO): lon −118.2817°, lat 37.2314°, alt 1222 m.
- **Important**: `DSA_110` is recognized by EveryBeam 0.7.4+ for automatic beam
  model detection.
- Optional: to make casacore resolve `DSA_110` by name, create a Measures
  overlay (`geodetic/Observatories`) and set `CASACORE_DATA` to that directory
