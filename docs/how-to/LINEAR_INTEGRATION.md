...
- **Sync TODO items to Linear** - Automatically create Linear issues from
  TODO.md items
...
**Important:** The `.linear_config.json` file is in `.gitignore` and will not be
...
- **One-way sync (TODO → Linear):** Currently syncs TODO.md to Linear, not
  reverse
...
- **No conflict resolution:** If both TODO.md and Linear are edited, manual sync
  needed
