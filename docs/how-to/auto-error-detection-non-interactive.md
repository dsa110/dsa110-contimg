...
Auto error detection has been moved **before** the interactive shell check in `~/.bashrc`, so it now works in both interactive and non-interactive shells.

### What Changed

The auto error detection block was moved from line ~261 (after the interactive check) to line 6 (before the interactive check):
