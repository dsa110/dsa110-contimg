...
Bash evaluates each command in a chain separately. The auto-error-detection system works by:

1. **Function wrappers** - Overrides specific commands (`python`, `pytest`, etc.)
2. **command_not_found_handle** - Only catches commands that don't exist (not useful for existing scripts)
...
For command chains, wrap the **entire chain** if you want error detection on all parts:
