...
This guide documents exactly how we generate a CASA VP table from the DSA-110
beam model HDF5, so anyone can reproduce it.
...
- Note: `phi_pts` maps to Az; Elevation is computed from polar angle as
  `El = 90° − theta`
...
- Temp complex image used to create it:
  `/stage/dsa110-contimg/vp/dsa110_vp_tmp.im`

Under the hood (`vp_builder.py`):

- Converts angles to degrees; computes elevation from `theta`.
- Builds the 4-pol complex Jones cube and a stable `coordsys` via
  `casatools.coordsys.newcoordsys(...)`.
- Registers the complex image with
  `vpmanager.setpbimage(compleximage=..., antnames=['*'])` and saves
  `saveastable()`.
...
- A-Projection (if available in your CASA build):
  ...
  - Note: some CASA builds attempt a built-in A-term for the telescope name
    first; if unrecognized, A-Projection may fail before using the VP.
- Alternative for Stokes I: `wproject` + scalar PB correction
  ...
  - Divide image by PB where `PB >= pblimit` (e.g., 0.25); for MT‑MFS, use
    wideband PB correction.

### Notes

- The VP encodes full 4‑pol Jones; you can supply it even if you only form
  Stokes I.
- For reproducibility, the CLI defaults are stable (center boresight, single
  channel, AZEL csys).
