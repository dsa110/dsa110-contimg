...
**There is NO way to guarantee error detection is enabled** for a random agent
running commands unless:
...
Non-interactive shells (`bash -c 'command'`) source **nothing** by default. They
only source `BASH_ENV` if it's set in the **environment** before bash starts.
...
If an agent starts a completely fresh session (non-login, non-interactive) and
`BASH_ENV` isn't set in the environment, **nothing will be sourced** and error
detection won't be enabled.
...
If you control how commands are executed (e.g., in a wrapper script or CI/CD
system), always set `BASH_ENV`:
...
**Without system-level configuration or modifying the execution environment,
there is NO way to guarantee error detection is enabled for random agents.**
...
1. **If you have root access:** Set `BASH_ENV` system-wide in `/etc/environment`
   or `/etc/profile.d/`
2. **If you don't have root:** Document clearly and accept that agents may need
   to source the setup script
