...
- **K-calibration default behavior change (2025-11-03)**: K-calibration is now **skipped by default** for DSA-110 following reassessment. See `docs/reports/K_CALIBRATION_NEED_REASSESSMENT.md` for full analysis. The earlier assessments (`CONSEQUENCES_OF_SKIPPING_K_CALIBRATION.md`, `DSA110_K_CAL_NEED_ASSESSMENT.md`) correctly identified consequences but failed to distinguish VLBI arrays (thousands of km baselines) from connected-element arrays (short baselines). DSA-110's 2.6 km maximum baseline means residual delays are < 0.5 ns (below decorrelation threshold) and are absorbed into complex gain calibration, following VLA/ALMA practice. Use `--do-k` flag to explicitly enable K-calibration if needed (rarely required).
...
- **K-calibration is NOT needed for DSA-110 (2025-11-03)**: K-calibration (delay calibration) is **skipped by default** for DSA-110. Following VLA/ALMA practice for connected-element arrays with short baselines (< 100 km), DSA-110's 2.6 km maximum baseline means residual delays are < 0.5 ns (decorrelation threshold) and are absorbed into complex gain calibration. K-calibration is primarily needed for VLBI arrays (thousands of km baselines, independent atomic clocks). Use `--do-k` flag to explicitly enable K-calibration if ever needed. **This is now the default behavior** - the CLI uses `--do-k` to opt-in instead of `--skip-k` to opt-out.
- **Proper transit time calculation (2025-11-02)**: For end-to-end testing that reflects production pipeline behavior, always use the proper transit finding method: (1) Load calibrator coordinates via `load_vla_catalog()` (SQLite preferred), (2) Calculate transit times using `previous_transits(ra_deg=..., start_time=Time.now(), n=5)` from `dsa110_contimg.calibration.schedule`, (3) Calculate search window (±30-60 minutes around transit), (4) **CRITICAL: Verify data exists on disk** using `find_subband_groups()` from `dsa110_contimg.conversion.strategies.hdf5_orchestrator` - this must be part of the default workflow before proceeding to MS generation, (5) Use `hdf5_orchestrator` CLI with `--start-time` and `--end-time` flags (this is what the streaming pipeline uses). **Never use simplified time-based searches** (e.g., assuming 0834 transits at 08:34 UTC) - this bypasses actual pipeline components and doesn't test production workflow. The proper method verifies coordinates, uses astronomical calculations, verifies data availability, and aligns with streaming pipeline components (`hdf5_orchestrator.find_subband_groups()`). **SEE ALSO**: `docs/howto/FIND_CALIBRATOR_TRANSIT_DATA.md` for comprehensive guide on using `CalibratorMSGenerator` to find transits and get HDF5 file lists.
...
#### Stage 4: Calibration (BP → G, optional K)
...
2. **Delay Calibration (K-table) - Optional:**
   - `solve_delay()` - Frequency-independent delays per antenna
   - **Disabled by default** for DSA-110 (connected-element array practice)
   - Use `--do-k` flag to explicitly enable if needed
   - Solution interval: `'inf'` (entire scan) or `'60s'` (fast mode)
   - Reference antenna selection via composite score (phase stability + SNR)
3. **Bandpass Calibration (BP-table) - Standard:**
