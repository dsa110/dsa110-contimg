...
### Option A: Quick Calibration (Fast Mode)

For a quick 5-minute image, use fast calibration mode:
...
- Uses fast mode with time/channel binning
- Skips K-calibration (delay) by default (appropriate for DSA-110 short baselines)
- Creates calibration tables: `.bpcal`, `.gpcal`
- Applies calibration to `CORRECTED_DATA` column
...
- `--quick`: Enables quick-look mode (reduces `imsize` and `niter` automatically)
