...
1. **Separate code from data**: Code in `/data/dsa110-contimg/`, data in
   `/stage/dsa110-contimg/`
2. **Separate fast from slow**: Hot data on `/stage/` (fast SSD),
   cold/persistent on `/data/` or archive
3. **Clear lifecycle**: Incoming :arrow_right: Processing :arrow_right: Archive :arrow_right: Purge
4. **Predictable paths**: Standard naming conventions for automated discovery
5. **Production-ready**: Support 24/7 streaming operations with minimal human
   intervention
...
**Note:** tmpfs staging is now default, provides 3-5x speedup over SSD-only
writes.
...
**CRITICAL:** No automatic deletion of data at this time. All data retained
indefinitely until archival/backup strategy finalized.
...
**Note:** Once archival strategy to `/data/` or external storage is established,
retention policies can be re-evaluated.
...
- Compress images: FITS :arrow_right: FITS.fz (rice compression, ~50% reduction)
...
## Migration Plan (Current :arrow_right: Recommended)
...
2. **Flat directory structure**: :check: COMPLETED - Organized MS files by date and
   type (`calibrators/`, `science/`, `failed/`)
...
5. **Missing directories**: :check: COMPLETED - Created `science/`, `calibrators/`,
   `failed/` structure
6. **No archive mechanism**: Set up tape/cold storage workflow (TODO)
7. **Storage location registration**: :check: COMPLETED - Base directories registered
   in `storage_locations` table, individual paths tracked in `ms_index` and
   `cal_registry`
...
- Delay calibration (K table) - **Optional**, disabled by default for DSA-110
  (use `--do-k` to enable)
...
- Calibration tables use underscore suffixes (`_bpcal`, `_gpcal`, `_2gcal`)
  because CASA tables are directories, not files
...
- `ms_files` :arrow_right: `/stage/dsa110-contimg/ms/`
- `calibration_tables` :arrow_right: `/stage/dsa110-contimg/ms/calibrators/`
- `science_ms` :arrow_right: `/stage/dsa110-contimg/ms/science/`
- `failed_ms` :arrow_right: `/stage/dsa110-contimg/ms/failed/`
- `images` :arrow_right: `/stage/dsa110-contimg/images/`
- `mosaics` :arrow_right: `/stage/dsa110-contimg/mosaics/`
...
2. **Retention policy:** INDEFINITE - no automatic deletion until archive
   established
3. **Calibration frequency:** BP/full every 24h, gain every 1h using NVSS sky
   models
...
**Summary:** The current directory structure is functional but lacks
organization for automated operations. The recommended architecture provides
clear data lifecycle, automated cleanup, and production-ready organization for
