...
> **:warning: Migration Note:** The frontend is being rewritten. The new `frontend/`
...
> **:lightbulb: Quick Start Guide:** For a simplified task-oriented guide, see
...
:check: **Index once, search anytime**: `python cli.py index-frontend`  
:check: **Find components**: `python cli.py search-components "query"`  
:check: **Find hooks**: `python cli.py search-hooks "query"`  
:check: **Track API usage**: `python cli.py find-api "/api/status"`  
:check: **Incremental updates**: Only changed files are re-indexed  
:check: **Shared cache**: Minimizes OpenAI API costs
