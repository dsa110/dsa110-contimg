...
# :check: Permissions fixed successfully
```
...
```bash
# Test write access
touch /path/to/file.ms/test_write && rm /path/to/file.ms/test_write && echo ":check: Write access OK"
```
...
1. :check: **Automatic:** `selfcal.py` fixes permissions automatically
2. :check: **Manual:** Use `fix_ms_permissions.sh` script
3. :check: **Prevention:** Run as correct user from the start
