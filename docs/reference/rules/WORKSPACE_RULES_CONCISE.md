...
**Purpose:** Ultra-concise version for "Always Applied Workspace Rules"
section  
**Full details:** `.cursor/rules/critical-requirements.mdc` and
...
Add this concise version to the workspace configuration's "Always Applied
Workspace Rules":
...
4. ✅ **Works even if agents skip reading files** - It's in their context
   automatically

**Trade-offs:**

- ⚠️ Requires workspace config edit (one-time setup)
- ⚠️ Less flexible than referencing a file (but you can update the file and
  re-add to config)
...
1. **Copy the concise version above** into workspace "Always Applied Workspace
   Rules"
2. **Place casa6 requirement FIRST** (before other rules, before everything)
3. **Test:** Have an agent run a Python script - they should use casa6
   automatically
4. **Monitor:** If agents still use wrong Python, make the rule even more
   explicit
...
- `.cursor/rules/critical-requirements-short.mdc` - Concise version for
