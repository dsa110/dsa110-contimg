...
Migrated catalog query fallback notifications from `warnings.warn()` to `logger.warning()` for better consistency with the rest of the pipeline and improved visibility in production environments.
...
1. **Consistency**: Matches the logging pattern used throughout the rest of the pipeline
2. **Visibility**: Automatically captured in log files and production monitoring
3. **Configurability**: Can be filtered/configured via standard logging configuration
...
- [Fallback Notification Analysis](fallback_notification_analysis.md) - Analysis of fallback notification mechanisms
