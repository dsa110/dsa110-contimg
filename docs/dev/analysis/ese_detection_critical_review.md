...
This document provides a critical review of the ESE detection implementation
based on scientific literature and best practices.
...
**Issue**: Two different formulas for `sigma_deviation` existed in the codebase
(FIXED 2025-11-12):
...
This calculates: **max deviation from mean in units of standard deviation** ✓
(Correct)
...
Now calculates: **max deviation from mean in units of standard deviation** ✓
(Correct - Fixed)
...
**Issue**: ESEs show frequency-dependent variability, but we only analyze
single-frequency flux.
...
The ESE detection implementation is **generally sound** with good architecture
and integration. However, there is a **critical bug** in the sigma_deviation
calculation that must be fixed immediately. The inconsistency between automatic
and manual detection paths will lead to incorrect results.
