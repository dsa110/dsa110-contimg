...
- Tests time window calculations (±25 minutes default, ±50 minutes for full
  group)
...
- `test_group_id_generation_from_timestamp` - Group ID generation from MS
  timestamp
...
1. **Database Schema:** Used `ensure_products_db()` instead of manual table
   creation
2. **Time Window Calculation:** Adjusted time windows to account for MS file
   spacing (5 minutes apart)
3. **Mock Patches:** Fixed patch paths to match actual import locations
4. **Dataclass Serialization:** Changed from `model_dump()` to `asdict()` for
   dataclass serialization
5. **DataRecord Constructor:** Added all required fields for DataRecord
   initialization
...
All unit tests are passing and provide comprehensive coverage of the newly
