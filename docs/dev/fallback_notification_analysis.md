...
When SQLite databases are unavailable and the system falls back to CSV, users
are notified via Python's `warnings.warn()`:
...
2. **No Logging Integration**: Warnings are not automatically captured by the
   logging system unless explicitly configured
...
4. **Inconsistent with Rest of Pipeline**: Most of the pipeline uses
   `logger.warning()` for warnings, but catalog fallbacks use `warnings.warn()`
...
1. **Immediate**: Replace `warnings.warn()` with `logger.warning()` in catalog
   query functions
