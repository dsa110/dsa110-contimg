...
All backend API endpoints have been successfully tested with real data. Test
...
2. **Dataclass Error:** Fixed field ordering in `event_bus.py`
   ...
   - Fixed: `PhotometryMeasurementCompleted`, `ESECandidateDetected`,
     `CalibrationSolved`
...
- Disk space check failing (expected - `/stage/dsa110-contimg` doesn't exist in
  test environment)
- Health status shows "degraded" due to disk space check (doesn't affect
  functionality)
