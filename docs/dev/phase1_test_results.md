...
1. **`scripts/test_dlq_endpoints.py`** - Python script to create test DLQ items
   ...
   - Items: ese_detection.detect_candidates, calibration_solve.solve_gain, photometry.measure_flux
...
- [x] `GET /api/health/summary` - Returns comprehensive health summary
  - Status: "degraded" (expected - disk_space check failing due to missing /stage directory)
  - Contains all required fields: status, timestamp, checks, circuit_breakers, dlq_stats
...
- [x] `GET /api/operations/dlq/items` - Returns list of items
  ...
  - Each item has: id, component, operation, error_type, error_message, context, created_at, retry_count, status
...
- [ ] `POST /api/operations/circuit-breakers/{name}/reset` - Needs manual testing
...
1. **Item ID 1:**
   ...
   - Context: {min_sigma: 5.0, source_id: "test_source_001", image_id: "test_image_001"}
...
- [x] `src/dsa110_contimg/pipeline/event_bus.py` - Fixed dataclass field ordering issue

### Issues Fixed

1. **Import Error:** Removed non-existent `StageResult` from `pipeline/__init__.py`
...
**Completed:** ~60%
**Remaining:** Frontend testing, POST endpoint testing, error scenarios
...
**Known Issues:**
- Disk space check failing (expected - `/stage/dsa110-contimg` directory doesn't exist in test environment)
- Health status shows "degraded" due to disk space check (not critical for testing)
