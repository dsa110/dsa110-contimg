...
Migrated all legacy CSV-direct catalog reading code to use SQLite-first query
functions. All catalog queries now prioritize SQLite databases (~170× faster)
with automatic CSV fallback.
...
- **Functions**:
  ...
  - `create_nvss_overlay()`
- **Change**: Replaced `read_nvss_catalog()` → `query_nvss_sources()` (3
  instances)
...
- `src/dsa110_contimg/calibration/catalogs.py` - Defines `read_nvss_catalog()`
  (used as fallback)
- `src/dsa110_contimg/catalog/query.py` - Uses CSV fallback when SQLite
  unavailable
...
3. Consider deprecating `read_nvss_catalog()` for direct use (keep only for
   fallback)
