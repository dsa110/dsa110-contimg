...
| ----------------------- | ------------------- | ---------------- | -------- | ------ |
| Single group conversion | 2m 40s              | 2m 08s (mean)    | < 2m 30s | :check:     |
| Per-group range         | —                   | 1m 18s - 2m 31s  | —        | :check:     |
| Batch Time() conversion | 2.73ms              | 0.12ms           | —        | :check:     |
| MS file size            | —                   | ~5.1 GB          | —        | :check:     |

**Notes**:

- Times include full pipeline: HDF5 load :arrow_right: phase :arrow_right: write :arrow_right: concatenate :arrow_right:
  configure :arrow_right: validate
...
## Phase 1: Foundation (:check: COMPLETED)
...
## Phase 2: Numba Integration (:warning: PARTIALLY REVERTED)
...
**Status**: :warning: REVERTED to astropy
...
- [x] Profile astropy overhead in `get_meridian_coords()` :arrow_right: **5.15ms/call**
- [x] Add fast path using `approx_lst_jit()` :arrow_right: **0.02ms/call**
...
## Phase 3: Memory-Mapped I/O Integration (:warning: REVISED)
...
### 3.1 Fast File Validation (:cross: DEPRIORITIZED)
...
## Phase 4: Parallel Loading Integration (:check: COMPLETED)
...
## Phase 5: Pre-allocation Patterns (:check: COMPLETED)
...
2. Batch JD:arrow_right:MJD conversion (single `Time()` call instead of per-iteration)
...
- [x] Batch convert JD:arrow_right:MJD with single `Time()` call
...
## Phase 6: Measurement & Validation (:check: COMPLETED)
