...
Optimized NVSS catalog queries by migrating from CSV-based loading to SQLite databases with spatial indexing. This provides a **170× performance improvement** (1.2s → 7ms per query).
...
SQLite databases are created per declination strip using `build_nvss_strip_db()`:
...
**Indexes:**
- `idx_radec`: Composite index on `(ra_deg, dec_deg)` - optimal for spatial queries
...
This approach balances speed (indexed SQL queries) with accuracy (exact angular separation).
...
- `/data/dsa110-contimg/state/catalogs/nvss_dec+54.6.sqlite3` (17MB, 129k sources)
...
1. **Additional Declination Strips**: Build SQLite databases for other declination ranges as needed
2. **Query Optimization**: Consider using R-tree indexes for even faster spatial queries
3. **Caching**: Add query result caching for frequently accessed fields
4. **Parallel Queries**: Support batch queries for multiple fields simultaneously
