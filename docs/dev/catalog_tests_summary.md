...
1. **`test_query_nvss_sources.py`** (Updated)
   ...
   - `TestQueryNVSSSourcesCSVFallback` - CSV fallback when `use_csv_fallback=True` (3 tests)
   - `TestQueryNVSSSourcesNoFallback` - Default behavior when `use_csv_fallback=False` (3 tests) ✨ NEW
   ...
   - `TestQueryNVSSSourcesSmoke` - Smoke tests including CSV fallback disabled (7 tests)
...
- **Location**: `TestQueryNVSSSourcesSmoke` class in `test_query_nvss_sources.py`
- **Rationale**:
