...
(function() {
        ...
        elements.forEach((el) => {
            ...
            if (originalText && originalText.trim().length > 0) {
                ...
                if (sanitized !== originalText) {
                    console.log('[Mermaid Init] Immediate sanitization: sanitized element, length:', originalText.length, '->', sanitized.length);
        ...
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.nodeType === 1) { // Element node
                    ...
                    // Also check children
                    const mermaidElements = node.querySelectorAll ? node.querySelectorAll('.mermaid:not([data-sanitized])') : [];
    ...
    function renderDiagrams() {
        ...
        if (typeof window.mermaid.run !== 'function') {
            console.log('[Mermaid Init] mermaid.run() not available, mermaid methods:', Object.keys(window.mermaid));
            return false;
        }
        ...
        elementsToProcess.forEach((el, idx) => {
            ...
            if (originalText) {
                const sanitized = sanitizeMermaidText(originalText);
                if (sanitized !== originalText) {
                    console.log(`[Mermaid Init] Sanitized element ${idx}: decoded HTML entities and/or converted Unicode`);
        ...
        console.log('[Mermaid Init] Calling mermaid.run() with', elementsToProcess.length, 'elements...');
        ...
        if (elementsToProcess.length > 0) {
            ...
            console.log('  - Status:', visibleCount === elementsToProcess.length ? '✅ ALL VISIBLE' : 
                       visibleCount > 0 ? `⚠️  PARTIAL: ${visibleCount}/${elementsToProcess.length}` : '❌ NONE VISIBLE');
        }
        ...
        mermaid.run({
            ...
            elementsToProcess.forEach((el) => {
                ...
                if (svg) {
                    ...
                    if (rect.width === 0 || rect.height === 0) {
                        console.log('[Mermaid Init] Fixing SVG dimensions (current:', rect.width, 'x', rect.height, ')');
                        ...
                        if (hasViewBox && viewBox) {
                            ...
                            if (vbParts.length >= 4) {
                                ...
                                if (vbWidth > 0 && vbHeight > 0) {
                                    ...
                                    console.log('[Mermaid Init] Set dimensions from viewBox:', vbWidth, 'x', vbHeight);
                                }
                            }
                        }
                        ...
                        try {
                            ...
                            if (bbox.width > 0 && bbox.height > 0) {
                                ...
                                console.log('[Mermaid Init] Set dimensions from getBBox():', bbox.width, 'x', bbox.height);
                            } else {
            ...
            setTimeout(() => {
                ...
                console.log('[Mermaid Init] After dimension fix:', visibleAfterFix, '/', elementsToProcess.length, 'visible');
            }, 100);
    ...
    function sanitizeAllMermaidElements() {
        ...
        console.log('[Mermaid Init] Sanitizing', elements.length, 'mermaid elements before Mermaid processes them');
        ...
        elements.forEach((el, idx) => {
            ...
            if (originalText.includes('font-family') && originalText.includes('fill:')) {
                ...
                if (match) {
                    ...
                    const diagramMatch = html.match(/(flowchart|graph|sequenceDiagram|classDiagram|stateDiagram|erDiagram|gantt|pie|gitgraph|journey|requirement|C4Context|C4Container|C4Component|C4Dynamic|C4Deployment)[\s\S]{0,5000}/);
                    if (diagramMatch) {
            ...
            if (originalText && originalText.length > 0) {
                const sanitized = sanitizeMermaidText(originalText);
                if (sanitized !== originalText) {
                    console.log(`[Mermaid Init] Pre-sanitized element ${idx}: decoded HTML entities and/or converted Unicode`);
    ...
    function interceptInitialize() {
        if (window.mermaid && typeof window.mermaid.initialize === 'function' && !initializeIntercepted) {
