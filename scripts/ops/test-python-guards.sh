...
echo ":test_tube: Testing Python version guards..."
echo ""
...
if "$CASA6_PYTHON" "$PROJECT_ROOT/scripts/python-version-guard.py" >/dev/null 2>&1; then
    echo -e "${GREEN}:check: PASS${NC}: CASA6 Python works"
else
    echo -e "${RED}:cross: FAIL${NC}: CASA6 Python failed"
    exit 1
fi
...
if /usr/bin/python2.7 "$PROJECT_ROOT/scripts/python-version-guard.py" >/dev/null 2>&1; then
    echo -e "${RED}:cross: FAIL${NC}: Python 2.7 was not blocked"
    exit 1
else
    echo -e "${GREEN}:check: PASS${NC}: Python 2.7 correctly blocked"
fi
...
if /usr/bin/python3.6 "$PROJECT_ROOT/scripts/python-version-guard.py" >/dev/null 2>&1; then
    echo -e "${RED}:cross: FAIL${NC}: Python 3.6 was not blocked"
    exit 1
else
    echo -e "${GREEN}:check: PASS${NC}: Python 3.6 correctly blocked"
fi
...
if /usr/bin/python3.6 -c "import sys; sys.path.insert(0, 'src'); from dsa110_contimg.photometry.cli import *" >/dev/null 2>&1; then
    echo -e "${RED}:cross: FAIL${NC}: Script did not block Python 3.6"
    exit 1
else
    echo -e "${GREEN}:check: PASS${NC}: Script correctly blocked Python 3.6"
fi
...
if "$CASA6_PYTHON" -c "import sys; sys.path.insert(0, 'src'); from dsa110_contimg.photometry.cli import *; print('OK')" >/dev/null 2>&1; then
    echo -e "${GREEN}:check: PASS${NC}: Script works with CASA6 Python"
else
    echo -e "${YELLOW}:warning:  WARNING${NC}: Script import failed (may be due to missing dependencies)"
fi
...
if "$PROJECT_ROOT/scripts/validate-python-version.sh" --pre-commit >/dev/null 2>&1; then
    echo -e "${GREEN}:check: PASS${NC}: Validation script works"
else
    echo -e "${YELLOW}:warning:  WARNING${NC}: Validation script had issues (check output)"
fi
