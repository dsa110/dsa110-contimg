...
echo ":wrench: Setting up Python environment for dsa110-contimg..."

# Verify CASA6 Python exists
if [ ! -x "$CASA6_PYTHON" ]; then
    echo -e "${RED}:cross: ERROR: CASA6 Python not found at $CASA6_PYTHON${NC}"
...
if echo "$CASA6_VERSION" | grep -q "Python 3.11"; then
    echo -e "${GREEN}:check: CASA6 Python found:${NC} $CASA6_VERSION"
else
    echo -e "${YELLOW}:warning:  WARNING: CASA6 Python version may not be 3.11.x${NC}"
...
if [[ ":$PATH:" != *":$CASA6_BIN_DIR:"* ]]; then
    ...
    echo -e "${GREEN}:check: Added CASA6 bin to PATH${NC}"
else
    echo -e "${GREEN}:check: CASA6 bin already in PATH${NC}"
fi

# Set CASA6_PYTHON environment variable
export CASA6_PYTHON
echo -e "${GREEN}:check: Set CASA6_PYTHON=$CASA6_PYTHON${NC}"
...
if command -v python3 >/dev/null 2>&1; then
    ...
    if [ "$ACTUAL_PYTHON3" = "$CASA6_PYTHON" ]; then
        echo -e "${GREEN}:check: python3 now points to CASA6:${NC} $ACTUAL_VERSION"
    else
        echo -e "${YELLOW}:warning:  WARNING: python3 points to:${NC} $ACTUAL_PYTHON3"
...
echo ""
echo ":search: Checking for forbidden Python versions..."
FORBIDDEN_FOUND=0
...
for py in python2 python2.7 python3.6; do
    if command -v "$py" >/dev/null 2>&1; then
        ...
        if [[ "$PY_PATH" == /usr/bin/* ]]; then
            echo -e "${YELLOW}:warning:  System $py found:${NC} $PY_PATH ($PY_VERSION)"
...
echo ""
echo -e "${GREEN}:check: Python environment setup complete!${NC}"
echo ""
echo "Usage:"
echo "  :bullet: Use: \$CASA6_PYTHON script.py"
echo "  :bullet: Or: python3 script.py (if PATH is set correctly)"
echo "  :bullet: Verify: python3 --version (should show 3.11.x)"
echo ""
