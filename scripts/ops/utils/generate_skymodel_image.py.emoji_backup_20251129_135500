...
def main():
    ...
    parser.add_argument(
        "--radius", type=float, default=0.2, help="Radius in degrees (for NVSS, default: 0.2)"
    )
    parser.add_argument(
        "--min-flux-mjy",
        type=float,
        default=10.0,
        help="Minimum flux in mJy (for NVSS, default: 10.0)",
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Output base path (will add .fits and .png)"
    )
    parser.add_argument(
        "--image-size",
        type=int,
    ...
    parser.add_argument(
        "--pixel-scale", type=float, default=5.0, help="Pixel scale in arcseconds (default: 5.0)"
    )
    parser.add_argument(
        "--beam-fwhm", type=float, help="Beam FWHM in arcseconds (optional, for convolution)"
    )
    ...
    if args.sky_model:
        ...
        print(
            f"Creating NVSS sky model (center: {args.center_ra}°, {args.center_dec}°, radius: {args.radius}°)"
        )
