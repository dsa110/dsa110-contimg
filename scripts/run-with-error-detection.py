...
def is_false_positive_line(line: str, pattern: str) -> bool:
    ...
    line_no_comment = re.sub(
        r'/\*.*?\*/', '', line_no_comment, flags=re.DOTALL)

    # If pattern only in comment portion, it's a false positive
    if re.search(pattern, line, re.IGNORECASE | re.MULTILINE) and not re.search(pattern, line_no_comment, re.IGNORECASE | re.MULTILINE):
        return True
    ...
    if re.search(pattern, line_no_comment, re.IGNORECASE | re.MULTILINE):
        ...
        if re.search(r'^\s*(var|let|const|int|str|bool|float|list|dict|set)\s+.*' + pattern, line, re.IGNORECASE):
            return True
...
def check_test_results(output: str, exit_code: int, command: str) -> Tuple[bool, Optional[str]]:
    ...
    if junit_file and os.path.exists(junit_file):
        ...
        failed_tests = len([l for l in output.split('\n')
        ...
        if passed == 0 and exit_code != 0 and failed == 0 and errors == 0:
            warning(
                f"No tests executed and exit code is {exit_code} (might be setup error)")
...
def _run_with_pexpect(command: str, log_fd, error_lines: List[str], output_lines: List[str]) -> int:
    ...
    child = pexpect.spawn(
        '/bin/bash', ['-c', command], encoding='utf-8', timeout=None)
    ...
    while True:
        try:
            ...
            try:
                ...
                index = child.expect(
                    [pexpect.EOF, pexpect.TIMEOUT, r'.+'], timeout=0.1)
                ...
                if index == 0:  # EOF - process finished
                    ...
                    line = child.before + \
                        child.match.group(0) if child.match else child.before
                    if line.strip():
                        print(line.rstrip('\n\r'))
                        log_fd.write(line)
                        log_fd.flush()
                        ...
                        if error_result:
                            ...
                            error_lines.append(f"Line: {error_line}")
                            error(
                                f"Error pattern '{pattern}' detected in real-time!")
                        ...
                        if not error_result:
                            ...
                            if warning_result:
                                ...
                                error_lines.append(
                                    f"Warning Line: {warning_line}")
                                error(
                                    f"Warning pattern '{pattern}' detected in real-time!")
                                error(
                                    "Terminating command immediately due to warning...")
                                errors_detected = True
    ...
    if not errors_detected:
        if child.isalive():
            child.wait()
        exit_code = child.exitstatus if child.exitstatus is not None else (
            child.signalstatus if child.signalstatus else 0)
    else:
...
def _run_with_subprocess(command: str, log_fd, error_lines: List[str], output_lines: List[str]) -> int:
...
def main():
    ...
    if PEXPECT_AVAILABLE:
        try:
            exit_code = _run_with_pexpect(
                command, log_fd, error_lines, output_lines)
            if error_lines:
                errors_detected = True
        except Exception as e:
