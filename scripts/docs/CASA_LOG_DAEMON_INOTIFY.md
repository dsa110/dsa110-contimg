...
This new daemon uses `inotifywait` (kernel-level inotify) to monitor **all
directories recursively** with minimal CPU and memory usage.
...
2. **`casa_log_daemon_inotify.py`** - Python version (better logging/error
   handling)
3. **`casa-log-daemon-inotify.service`** - Systemd service for bash version
4. **`casa-log-daemon-inotify-python.service`** - Systemd service for Python
   version
...
Check that it gets moved to `/data/dsa110-contimg/state/logs/` within a few
seconds.
