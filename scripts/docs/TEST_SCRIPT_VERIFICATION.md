...
**Date:** 2025-01-15 **Script:** `scripts/test_pipeline_end_to_end.sh`
...
1. **Synthetic data** creates files: `2025-01-15T12:00:00_sb00.hdf5` through
   `sb03.hdf5`
...
4. **Time range** `2025-01-15T12:00:00` to `2025-01-15T12:01:00` matches
   1-minute duration
5. **Caltables** are created with extensions: `.bpcal`, `.gcal` (`.kcal` if
   `--do-k`)
6. **Imaging** creates `.image` directory, not `.fits` (skipped with
   `--skip-fits`)
...
All command syntax has been verified against actual source code. The script
should work correctly for end-to-end pipeline testing.
