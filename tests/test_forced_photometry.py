...
if len(results) == 0:
    ...
    # Summary statistics
    ratio_errors = [
        r["ratio_error"] for r in relative_results if np.isfinite(r["ratio_error"])
    ]
    if ratio_errors:
...
if len(results) > 0:
    ...
    if len(relative_results) > 0:
        ...
        if len(valid_snr_indices) > 0:
            ...
            valid_errors_snr = [
                ratio_errors_pct[i]
                for i in valid_snr_indices
                if i < len(ratio_errors_pct)
            ]
...
measured_fluxes = [
    r["result"].peak_jyb if np.isfinite(r["result"].peak_jyb) else np.nan
    for r in results
]
flux_errors = [
...
ax1.errorbar(
    expected_fluxes, measured_fluxes, yerr=flux_errors, fmt="o", capsize=5, capthick=2
)
ax1.plot(
...
ax2.fill_between(
    [0, max(expected_fluxes)], 0.9, 1.1, alpha=0.2, color="green", label="±10%"
)
