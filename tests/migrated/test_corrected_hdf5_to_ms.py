#!/usr/bin/env python3
"""
Corrected HDF5 to MS Conversion

This script implements the corrected HDF5 to MS conversion using the proper
CASA table descriptor format based on examination of existing MS files.
"""
...
def create_ms_from_hdf5_data_corrected(hdf5_data: dict, ms_path: str) -> bool:
    """
    Create MS file from HDF5 data using corrected CASA table tools.
    ...
    try:
        ...
        # Create corrected table descriptor based on existing MS structure
        tabledesc = {
            'ANTENNA1': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'ANTENNA2': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'ARRAY_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'DATA_DESC_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'EXPOSURE': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'FEED1': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'FEED2': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'FIELD_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'FLAG': {'valueType': 'boolean', 'ndim': 2, 'option': 0, 'shape': [n_freqs, n_pols]},
            'FLAG_CATEGORY': {'valueType': 'boolean', 'ndim': 3, 'option': 0, 'shape': [1, n_freqs, n_pols]},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0},
            'INTERVAL': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'OBSERVATION_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'PROCESSOR_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'SCAN_NUMBER': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'SIGMA': {'valueType': 'float', 'ndim': 1, 'option': 0, 'shape': [n_pols]},
            'STATE_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'TIME': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'TIME_CENTROID': {'valueType': 'double', 'ndim': 0, 'option': 0},
        ...
        # Create subtables
        create_antenna_table_corrected(ms_path, hdf5_data)
        create_spectral_window_table_corrected(ms_path, hdf5_data)
        create_polarization_table_corrected(ms_path, hdf5_data)
        create_field_table_corrected(ms_path, hdf5_data)
        create_observation_table_corrected(ms_path, hdf5_data)
        create_source_table_corrected(ms_path, hdf5_data)
        create_history_table_corrected(ms_path, hdf5_data)
...
def create_antenna_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the antenna table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            'NAME': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'STATION': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'TYPE': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'MOUNT': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'POSITION': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [3]},
            'OFFSET': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [3]},
            'DISH_DIAMETER': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0}
        }
...
def create_spectral_window_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the spectral window table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            'CHAN_FREQ': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [n_freqs]},
            'CHAN_WIDTH': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [n_freqs]},
            'EFFECTIVE_BW': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [n_freqs]},
            'RESOLUTION': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [n_freqs]},
            'TOTAL_BANDWIDTH': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'NET_SIDEBAND': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0}
        }
...
def create_polarization_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the polarization table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            'CORR_TYPE': {'valueType': 'int', 'ndim': 1, 'option': 0, 'shape': [n_pols]},
            'CORR_PRODUCT': {'valueType': 'int', 'ndim': 2, 'option': 0, 'shape': [n_pols, 2]},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0}
        }
...
def create_field_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the field table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            'NAME': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'CODE': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'TIME': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'NUM_POLY': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'SOURCE_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'DELAY_DIR': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [2]},
            'PHASE_DIR': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [2]},
            'REFERENCE_DIR': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [2]},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0}
        }
...
def create_observation_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the observation table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            'TELESCOPE_NAME': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'TIME_RANGE': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [2]},
            'OBSERVER': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'PROJECT': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'RELEASE_DATE': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'SCHEDULE_TYPE': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0}
        }
...
def create_source_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the source table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            ...
            'TIME': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'INTERVAL': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'SPECTRAL_WINDOW_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'NUM_LINES': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'NUM_TRANSITIONS': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'DIRECTION': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [2]},
            'PROPER_MOTION': {'valueType': 'double', 'ndim': 1, 'option': 0, 'shape': [2]},
            'FLAG_ROW': {'valueType': 'boolean', 'ndim': 0, 'option': 0}
        }
...
def create_history_table_corrected(ms_path: str, hdf5_data: dict):
    """Create the history table with corrected format."""
    try:
        from casatools import table
        ...
        tabledesc = {
            'OBSERVATION_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'TIME': {'valueType': 'double', 'ndim': 0, 'option': 0},
            'MESSAGE': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'PRIORITY': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'ORIGIN': {'valueType': 'string', 'ndim': 0, 'option': 0},
            'OBJECT_ID': {'valueType': 'int', 'ndim': 0, 'option': 0},
            'APPLICATION': {'valueType': 'string', 'ndim': 0, 'option': 0}
        }
...
async def test_corrected_hdf5_to_ms():
    """Test the corrected HDF5 to MS conversion."""
    try:
        logger.info("🚀 Starting Corrected HDF5 to MS Conversion Test")
        ...
        logger.info("🔧 Testing corrected HDF5 to MS conversion...")
        ...
        success = create_ms_from_hdf5_data_corrected(hdf5_data, ms_path)
...
if __name__ == "__main__":
    asyncio.run(test_corrected_hdf5_to_ms())
