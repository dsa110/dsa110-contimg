        ...
        try:
            ...
            if Path(casa6_python).exists():
                self.process = subprocess.Popen(
                    [casa6_python, '-m', 'mkdocs', 'serve', '-a', f'{self.host}:{self.port}'],
            ...
            print("ERROR: mkdocs command not found. Install with: pip install -r docs/requirements.txt")
            return False
    ...
    def check_for_errors(self) -> Tuple[bool, Optional[str], int]:
        ...
        try:
            ...
            for i, block in enumerate(mermaid_blocks, 1):
                ...
                if block.strip() and not any(keyword in block for keyword in ['graph', 'flowchart', 'sequenceDiagram', 'classDiagram', 'stateDiagram', 'erDiagram', 'gantt', 'pie', 'gitgraph']):
