...
def mode_baseline(args):
    ...
    for i, ref in enumerate(ref_sources[:10], 1):
        print(f"  {i}. {ref.nvss_name}: SNR={ref.snr_nvss:.1f}, "
    ...
    print(f"  MAD: {1.4826 * np.median(np.abs(measurements_arr - np.median(measurements_arr))):.4f} Jy/beam")
    ...
    with open(baseline_file, 'w') as f:
        ...
        for ref, meas in zip(ref_sources, measurements):
            if ref.is_valid:
                f.write(f"{ref.source_id},{ref.ra_deg:.6f},{ref.dec_deg:.6f},"
...
def mode_validate(args):
    ...
    if args.plot:
        ...
        ax.plot([min(baseline_vals), max(baseline_vals)], 
                [min(baseline_vals), max(baseline_vals)], 
                'r--', alpha=0.5, label='1:1')
        ...
        plt.tight_layout()
        plot_file = Path(args.image).parent / f"{Path(args.image).stem}_normalization_validation.png"
...
def main():
    ...
    parser.add_argument('--catalog', default='state/catalogs/master_sources.sqlite3',
                      help="Path to master_sources catalog")
    parser.add_argument('--baseline-file', default='state/photometry_baseline_0702.csv',
                      help="File to store/load baseline")
    parser.add_argument('--fov-radius', type=float, default=1.5,
                      help="FoV radius for reference search (deg)")
    parser.add_argument('--min-snr', type=float, default=50.0,
                      help="Minimum NVSS SNR for references")
    parser.add_argument('--max-refs', type=int, default=20,
                      help="Maximum number of references")
    parser.add_argument('--box', type=int, default=5,
                      help="Pixel box size for forced photometry")
    parser.add_argument('--annulus', nargs=2, type=int, default=[12, 20],
