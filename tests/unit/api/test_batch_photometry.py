...
class TestBatchPhotometryJobAdapter:
    ...
    def test_run_batch_photometry_job_success(
        self, mock_update, mock_measure, tmp_path
    ):
    ...
    @patch("dsa110_contimg.photometry.forced.measure_forced_peak")
    def test_run_batch_photometry_job_with_normalization(
        self, mock_measure, tmp_path
    ):
        ...
        with patch(
            "dsa110_contimg.photometry.normalize.query_reference_sources"
        ) as mock_query, patch(
    ...
    @patch("dsa110_contimg.photometry.forced.measure_forced_peak")
    def test_run_batch_photometry_job_failure_handling(
        self, mock_measure, tmp_path
    ):
    ...
    @patch("dsa110_contimg.photometry.forced.measure_forced_peak")
    def test_run_batch_photometry_job_cancellation(
        self, mock_measure, tmp_path
    ):
        ...
        assert done_count == len(fits_paths) * len(coordinates), \
