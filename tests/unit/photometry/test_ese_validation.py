...
def create_test_photometry_data():
    ...
    def _create_data(n_obs=5, base_flux=10.0, variability=0.0):
        ...
        for i in range(n_obs):
            flux = base_flux + variability * np.sin(i) + np.random.normal(0, 0.1)
...
class TestVariabilityMetrics:
    ...
    def test_chi_squared_calculation(self, create_test_photometry_data):
        ...
        df = create_test_photometry_data(n_obs=5, base_flux=3.0, variability=0.0)
    ...
    def test_chi_squared_zero_variance(self, create_test_photometry_data):
        ...
        df = create_test_photometry_data(n_obs=4, base_flux=1.0, variability=0.0)
        ...
        chi2 = np.sum(
            ((df["normalized_flux_jy"] - expected_flux) ** 2) / (df["normalized_flux_err_jy"] ** 2)
        )
        chi2_nu = chi2 / (len(df) - 1)

        # Assert: Should be approximately zero
        assert abs(chi2_nu) < 0.01, f"Expected ≈ 0.0 for zero variance, got {chi2_nu}"
    ...
    def test_chi_squared_missing_errors(self, create_test_photometry_data):
        ...
        df = create_test_photometry_data(n_obs=5, base_flux=10.0, variability=0.0)
        df.loc[2, "normalized_flux_err_jy"] = np.nan

        # Act: Filter out NaN errors
        valid_mask = np.isfinite(df["normalized_flux_err_jy"]) & (df["normalized_flux_err_jy"] > 0)
        df_valid = df[valid_mask]

        # Assert: Should handle gracefully
        assert len(df_valid) == 4, "Should filter out NaN error"
        assert not df_valid["normalized_flux_err_jy"].isna().any(), "No NaN errors should remain"
    ...
    def test_eta_metric_against_vast_tools(self, create_test_photometry_data):
        ...
        df_no_var = create_test_photometry_data(n_obs=3, base_flux=1.0, variability=0.0)
        ...
        assert abs(eta_no_var) < 0.01, f"Expected ≈ 0.0 for no variability, got {eta_no_var}"
    ...
    def test_eta_metric_edge_cases(self, create_test_photometry_data):
        ...
        # Single measurement
        df_single = create_test_photometry_data(n_obs=1, base_flux=1.0, variability=0.0)
        eta_single = calculate_eta_metric(df_single)
        assert eta_single == 0.0, "Single measurement should return 0.0"

        # Zero variance
        df_zero = create_test_photometry_data(n_obs=3, base_flux=1.0, variability=0.0)
    ...
    def test_v_metric_edge_cases(self):
        ...
        assert v_single == 0.0 or np.isnan(v_single), "Single measurement edge case"

        # Zero variance
        v_zero = calculate_v_metric(np.array([1.0, 1.0, 1.0]))
        assert abs(v_zero) < 0.01 or v_zero == 0.0, "Zero variance should return ≈ 0.0"
    ...
    def test_vs_metric_calculation(self):
        ...
        assert abs(vs_metric - expected) < 0.01, f"Expected ≈ {expected}, got {vs_metric}"
...
class TestESEDetectionPipeline:
    ...
    def test_end_to_end_detection_smoke(self, temp_products_db, create_known_ese_pattern):
        ...
        conn = sqlite3.connect(temp_products_db)
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM photometry WHERE source_id = ?", (source_id,))
...
class TestEdgeCases:
    ...
    def test_single_measurement(self, temp_products_db):
        ...
        conn = sqlite3.connect(temp_products_db)
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM photometry WHERE source_id = ?", ("SINGLE001",))
        count = cursor.fetchone()[0]
        conn.close()
