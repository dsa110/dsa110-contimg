...
class TestVariabilityCaching:
    ...
    def test_cache_invalidation(self, temp_products_db):
        ...
        assert stats is None or stats.get(
            'stale', False), "Cache should be invalidated"
    ...
    def test_cache_ttl_expiration(self, temp_products_db):
        ...
        stats = get_cached_variability_stats(
            "TTL001", temp_products_db, ttl_seconds=300)

        # Assert: Should be expired (return None or stale)
        assert stats is None or stats.get(
            'stale', False), "Expired cache should be stale"
    ...
    def test_cache_smoke(self, temp_products_db):
        ...
        for i in range(10):
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (f"SMOKE{i:03d}", 120.0 + i, 45.0,
                 10, 50.0, 5.0, 3.5, time.time())
            )
        conn.commit()
        conn.close()
        ...
        for i in range(10):
            stats = get_cached_variability_stats(
                f"SMOKE{i:03d}", temp_products_db)
