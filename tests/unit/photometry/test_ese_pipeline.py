...
def sample_photometry_data(temp_products_db):
    ...
    measurements = [
        ("image1.fits", 120.0, 45.0, 100.0, 0.05, 0.002,
         now - 86400 * 5, source_id, mjd_base - 5),
        ("image2.fits", 120.0, 45.0, 100.0, 0.06, 0.002,
         now - 86400 * 4, source_id, mjd_base - 4),
        ("image3.fits", 120.0, 45.0, 100.0, 0.07, 0.002,
         now - 86400 * 3, source_id, mjd_base - 3),
        ("image4.fits", 120.0, 45.0, 100.0, 0.08, 0.002,
         now - 86400 * 2, source_id, mjd_base - 2),
        ("image5.fits", 120.0, 45.0, 100.0, 0.12, 0.002, now -
         86400 * 1, source_id, mjd_base - 1),  # High flux
    ]
...
class TestUpdateVariabilityStatsForSource:
    ...
    def test_update_stats_success(self, temp_products_db, sample_photometry_data):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM variability_stats WHERE source_id = ?", (source_id,)
        )
        row = cursor.fetchone()
    ...
    def test_update_stats_idempotent(self, temp_products_db, sample_photometry_data):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM variability_stats WHERE source_id = ?", (source_id,)
        )
