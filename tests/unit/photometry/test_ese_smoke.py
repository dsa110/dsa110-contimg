...
def smoke_test_db(tmp_path):
    ...
    now = time.time()
    test_sources = [
        ("smoke_source_001", 120.0, 45.0, 100.0, 10, 50.0, 5.0, 45.0, 55.0, 2.5, 6.5, None, now, 60000.0, now),
        ("smoke_source_002", 121.0, 46.0, 150.0, 15, 75.0, 3.0, 70.0, 80.0, 1.8, 4.2, None, now, 60001.0, now),
        ("smoke_source_003", 122.0, 47.0, 200.0, 20, 100.0, 8.0, 90.0, 110.0, 3.2, 7.8, None, now, 60002.0, now),
    ]
...
class TestESEDetectionSmoke:
    ...
    def test_smoke_detect_verify_database(self, smoke_test_db):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM ese_candidates WHERE status = 'active'"
        )
