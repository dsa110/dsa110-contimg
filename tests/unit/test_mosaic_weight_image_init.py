...
class TestWeightImageInitialization:
    ...
    def test_output_paths_removed_before_defineoutputimage(
        ...
        with patch('casatools.linearmosaic', return_value=mock_lm):
            with patch('dsa110_contimg.mosaic.cli._calculate_mosaic_bounds') as mock_bounds:
                with patch('dsa110_contimg.mosaic.cli._create_common_coordinate_system') as mock_template:
                    with patch('dsa110_contimg.mosaic.coordinate_utils.filter_tiles_by_overlap') as mock_filter:
                        with patch('casatasks.imregrid') as mock_regrid:
                            # Setup mocks
                            mock_bounds.return_value = (
                                117.0, 125.0, 52.0, 56.0)
                            mock_template.return_value = (
                                "/fake/template.image", [1, 1, 100, 200])
                            ...
                            metrics_dict = {tiles[0]: Mock(
                                pb_path="/fake/pb1.image")}
    ...
    def test_both_image_and_weight_paths_removed(self):
        ...
        assert len(
            cleanup_calls) == 2, f"Cleanup should be called for both paths, got {len(cleanup_calls)}"
        assert cleanup_calls[0][1] == output_path, "First cleanup should be for output_path"
        assert cleanup_calls[1][1] == output_weight_path, "Second cleanup should be for output_weight_path"
        assert all(
            call[0] == 'rmtree' for call in cleanup_calls), "Both should use rmtree for directories"
