...
class TestCalibrationTableRegistration:
    ...
    @patch("dsa110_contimg.calibration.streaming.solve_calibration_for_ms")
    def test_calibration_tables_registered_on_success(
        self, mock_solve, mock_register
    ):
    ...
    @patch("dsa110_contimg.database.registry.register_set_from_prefix")
    @patch("dsa110_contimg.calibration.streaming.solve_calibration_for_ms")
    def test_calibration_tables_not_registered_on_failure(
        self, mock_solve, mock_register
    ):
...
class TestDatabaseUpdates:
    ...
    def test_has_calibrator_field_update(self, tmp_path):
        ...
        gid = "2025-01-15T12:00:00"
        conn.execute(
            "INSERT INTO ingest_queue (group_id) VALUES (?)", (gid,)
        )
        conn.commit()
        ...
        # Verify update
        cursor = conn.execute(
            "SELECT has_calibrator FROM ingest_queue WHERE group_id = ?", (gid,)
        )
