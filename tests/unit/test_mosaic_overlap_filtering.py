...
class MockCoordinateSystem:
    ...
    def toworld(self, pixel_coords):
        ...
        dec_rad = self._ref_val[0] + \
            (dec_pix - self._ref_pix[0]) * self._increment[0]
        ra_rad = self._ref_val[1] + \
            (ra_pix - self._ref_pix[1]) * self._increment[1]
    ...
    def topixel(self, world_coords):
        ...
        ra_pix = self._ref_pix[1] + \
            (ra_rad - self._ref_val[1]) / self._increment[1]
        dec_pix = self._ref_pix[0] + \
            (dec_rad - self._ref_val[0]) / self._increment[0]
...
class TestOverlapFiltering:
    ...
    def test_filter_tiles_by_overlap_basic(self, mock_casaimage):
        ...
        mock_casaimage.side_effect = [tile1_img,
                                      template_img, tile2_img, template_img]
