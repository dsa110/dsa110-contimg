...
1. **Bounds Calculation Fix**: Tests verify that pixel coordinate order is correct for 4D images (`[0, 0, y, x]` instead of `[y, x, 0, 0]`)

2. **Weight Image Initialization Fix**: Tests verify that output paths are cleaned up before calling `linearmosaic.defineoutputimage()`

3. **Coordinate System Centering**: Tests verify that templates are centered on calculated mosaic bounds
