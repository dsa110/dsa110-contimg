...
def measure_bandpass_quality(caltable_path: str) -> dict:
    ...
    try:
        with table(caltable_path, readonly=True) as cal_tb:
            ...
            amp_range = (float(np.min(amplitudes)), float(np.max(amplitudes))) if amplitudes.size > 0 else (0.0, 0.0)
...
def test_complete_workflow_produces_good_bandpass(ms_path, calibration_output_dir):
    ...
    if "error" in model_quality:
        ...
        print(f"  {status} Phase scatter acceptable (< {thresholds['model_data_phase_scatter_max']}°)")
    ...
    if "error" in uvw_status:
        ...
        print(f"  Phase center: RA={uvw_status.get('phase_center', (0,0))[0]:.6f}°, Dec={uvw_status.get('phase_center', (0,0))[1]:.6f}°")
    ...
    if "error" in bp_quality:
        ...
        checks = {
            "Flagging rate < 50%": bp_quality['flagging_rate'] < thresholds['bandpass_flagging_rate_max'],
            "SNR >= 3.0": bp_quality.get('median_snr') is None or bp_quality['median_snr'] >= thresholds['bandpass_median_snr_min'],
            "Amplitude in range": thresholds['bandpass_amplitude_range'][0] < bp_quality['median_amplitude'] < thresholds['bandpass_amplitude_range'][1],
        }
    ...
    if model_quality_ok and bp_quality_ok:
        ...
        print("\nEvidence:")
        print(f"  - MODEL_DATA phase scatter: {model_quality.get('phase_scatter_deg', 'N/A'):.1f}° (< 10° required)")
        print(f"  - Bandpass flagging rate: {bp_quality.get('flagging_rate', 'N/A'):.1%} (< 50% required)")
        ...
        if not model_quality_ok:
            print(f"  - MODEL_DATA phase scatter too high: {model_quality.get('phase_scatter_deg', 'N/A'):.1f}°")
        if not bp_quality_ok:
            ...
            if bp_quality.get('median_snr') and bp_quality['median_snr'] < thresholds['bandpass_median_snr_min']:
