...
class TestStageChainExecution:
    ...
    def test_catalog_setup_to_conversion_chain(self, config, initial_context):
        ...
        with patch("dsa110_contimg.pointing.utils.load_pointing", return_value={"dec_deg": 30.0}), \
             patch("dsa110_contimg.database.products.ensure_products_db"), \
             patch("dsa110_contimg.catalog.query.resolve_catalog_path", return_value=Path("/mock/catalog.db")), \
             patch("dsa110_contimg.catalog.builders.build_nvss_strip_db", return_value=Path("/mock/nvss.db")), \
             patch("dsa110_contimg.catalog.builders.build_first_strip_db", return_value=Path("/mock/first.db")), \
             patch("dsa110_contimg.catalog.builders.build_rax_strip_db", return_value=Path("/mock/rax.db")), \
             patch("dsa110_contimg.conversion.strategies.hdf5_orchestrator.convert_subband_groups_to_ms", return_value=["/mock/converted.ms"]):
    ...
    def test_conversion_to_calibration_chain(self, config, initial_context):
        ...
        with patch("dsa110_contimg.calibration.calibration.solve_calibration_tables", return_value={"K": "/mock/K.cal", "BA": "/mock/BA.cal"}):
    ...
    def test_calibration_to_imaging_chain(self, config, initial_context):
        ...
        context_with_cal = context_with_ms.with_output(
            "calibration_tables",
            {"K": "/mock/K.cal", "BA": "/mock/BA.cal"}
        )
        
        # Mock imports that happen inside execute methods
        with patch("dsa110_contimg.calibration.apply_service.apply_calibration"), \
             patch("dsa110_contimg.imaging.imaging.run_tclean", return_value="/mock/image.fits"):
    ...
    def test_imaging_to_validation_chain(self, config, initial_context):
        ...
        with patch("dsa110_contimg.qa.validation_suite.run_validation_suite", return_value={
            "status": "passed",
            "metrics": {"snr": 10.5, "rms": 0.001}
        }):
...
class TestStageOutputPropagation:
    ...
    def test_outputs_persist_through_multiple_stages(self, config):
        ...
        initial_context = PipelineContext(
            config=config,
            inputs={"input_path": "/mock/input.hdf5"}
        )
...
class TestOrchestratorWithRealStages:
    ...
    def test_orchestrator_executes_stage_chain(self, config, initial_context):
        ...
        with patch("dsa110_contimg.pointing.utils.load_pointing", return_value={"dec_deg": 30.0}), \
            ...
            assert result.status.value in ["completed", "partial"]  # May be partial if later stages not run
