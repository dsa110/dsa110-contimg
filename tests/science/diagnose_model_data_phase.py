...
def diagnose_model_data_phase(ms_path: str, cal_ra_deg: float, cal_dec_deg: float):
    ...
    with table(ms_path, readonly=True) as tb:
        ...
        expected_phase = 2 * np.pi * (u_coord * offset_ra_rad + v_coord * offset_dec_rad) / wavelength
        expected_phase = np.mod(expected_phase + np.pi, 2*np.pi) - np.pi  # Wrap to [-π, π]
        expected_phase_deg = np.degrees(expected_phase)
        
        print(f"   Source offset: ΔRA={offset_ra_rad*180/np.pi*3600:.2f} arcsec, "
        ...
        if np.std(phase_diff_deg) > 50:
            print(f"   ✗ PROBLEM: MODEL_DATA phase doesn't match expected phase structure")
            ...
            print(f"        3. MODEL_DATA was written before MS rephasing and not properly cleared")
    ...
    if comp_path.exists():
        ...
        print(f"   (Component list position should match calibrator: "
