...
def test_ft_uses_reference_dir_or_phase_dir(ms_path):
    ...
    try:
        ...
        with table(f"{test_ms}::FIELD", readonly=False) as field_tb:
            ...
            print(f"Set REFERENCE_DIR to position A: RA={position_a[0]*180/np.pi:.6f}°, Dec={position_a[1]*180/np.pi:.6f}°")
            print(f"Set PHASE_DIR to position B: RA={position_b[0]*180/np.pi:.6f}°, Dec={position_b[1]*180/np.pi:.6f}°")
