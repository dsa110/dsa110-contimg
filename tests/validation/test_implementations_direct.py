...
def test_performance_metrics():
    ...
    spec = importlib.util.spec_from_file_location(
        "performance", 
        Path("src/dsa110_contimg/utils/performance.py")
    )
    ...
    assert stats["test_operation"]["count"] == 5, f"Expected 5 calls, got {stats['test_operation']['count']}"
...
def test_error_context():
    ...
    spec = importlib.util.spec_from_file_location(
        "error_context",
        Path("src/dsa110_contimg/utils/error_context.py")
    )
...
def test_cache_stats():
    ...
    spec = importlib.util.spec_from_file_location(
        "ms_helpers",
        Path("src/dsa110_contimg/utils/ms_helpers.py")
    )
    ...
    print(f"  Flag validation cache: maxsize={flag_stats['maxsize']}, currsize={flag_stats['currsize']}")
    print()
...
def test_parallel_processing():
    ...
    spec = importlib.util.spec_from_file_location(
        "parallel",
        Path("src/dsa110_contimg/utils/parallel.py")
    )
...
def test_type_annotations():
    ...
    assert sig1.return_annotation != inspect.Signature.empty, "clear_ms_metadata_cache should have return annotation"
    # None type annotation can be None or NoneType
    assert sig1.return_annotation in (type(None), None), f"Expected None type, got {sig1.return_annotation}"
    
    sig2 = inspect.signature(ms_helpers.clear_flag_validation_cache)
    assert sig2.return_annotation != inspect.Signature.empty, "clear_flag_validation_cache should have return annotation"
    assert sig2.return_annotation in (type(None), None), f"Expected None type, got {sig2.return_annotation}"
...
def test_duplicate_function_fix():
    ...
    sort_key_files_lines = [i+1 for i, line in enumerate(lines) if re.search(r'\bdef sort_key_files\(', line)]
    ...
    assert sort_key_files_count == 1, f"Expected 1 sort_key_files function, found {sort_key_files_count}"
