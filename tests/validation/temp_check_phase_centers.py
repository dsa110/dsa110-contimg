...
print(
    f"  RA: {expected_ra_deg:.10f}° ({expected_coord.ra.to_string(unit=u.hour, precision=2, pad=True)})"
)
print(
    f"  Dec: {expected_dec_deg:.10f}° ({expected_coord.dec.to_string(unit=u.deg, precision=2, pad=True, alwayssign=True)})"
)
...
with table(ms_path + "/FIELD", readonly=True) as field:
    ...
    phase_dirs = field.getcol(
        "PHASE_DIR"
    )  # Shape: (nfields, 1, 2) - (RA, Dec) in radians
    ...
    if max_offset < 0.1:
        ...
        if "REFERENCE_DIR" in field.colnames():
            ...
            for i in range(min(3, nfields)):
                ...
                phase_coord = SkyCoord(
                    phase_ra * u.deg, phase_dec * u.deg, frame="icrs"
                )
                ref_phase_sep = ref_coord.separation(phase_coord).arcsec

                print(
                    f"  Field {i}: REFERENCE_DIR vs PHASE_DIR offset: {ref_phase_sep:.6f} arcsec"
                )

        print(f"\nNote: MODEL_DATA was created using ft() with component list.")
        print(
            f"ft() uses PHASE_DIR from the FIELD table, but may have phase center bugs."
        )
