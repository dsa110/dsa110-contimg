...
This directory contains test utilities for the DSA-110 continuum imaging pipeline.

## Refactored Utilities

### `convert_uvh5_refactored.py`

**Purpose**: Refactored version of old standalone converters that now uses production modules.
...
- `convert_uvh5_standalone.py` → `archive/tests/utils/convert_uvh5_standalone.py.archived`
- `convert_uvh5_simple.py` → `archive/tests/utils/convert_uvh5_simple.py.archived`

**Reason**: These contained duplicate implementations of `find_subband_groups` and core conversion logic that should use production modules instead.
