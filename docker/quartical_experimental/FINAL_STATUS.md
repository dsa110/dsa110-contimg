...
- **Fixed**: `_fix_field_phase_centers_from_times()` now uses `detect_casa_time_format()`
- **Fixed**: `api/routes.py` uses `extract_ms_time_range()`
- **Fixed**: `test_utils.py` handles both TIME formats
- **Impact**: Phase centers now calculated correctly (RA: ~170° → ~128°)
...
**What's Blocked**:
- ✗ CubiCal installation (sharedarray/Python 3.11 issue)
