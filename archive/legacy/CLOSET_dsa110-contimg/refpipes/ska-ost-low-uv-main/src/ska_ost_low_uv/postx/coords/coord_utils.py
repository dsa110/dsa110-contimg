...
def lmn_to_altaz(l: np.ndarray, m: np.ndarray, n: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Convert direction cosines l,m,n into alt, az.

    Args:
        l (np.ndarray): Direction cosine l = cos(theta_x)
        m (np.ndarray): Direction cosine m = cos(theta_y)
        n (np.ndarray): Corresponding unit vector component n = sqrt(1 - l^2 - m^2)

    Returns:
        alt (np.ndarray): altitude angle in radians.
        az (np.ndarray): azimuth angle in radians.
    """
    az = np.arctan2(l, m)
    alt = np.arcsin(n)
    return alt, az
