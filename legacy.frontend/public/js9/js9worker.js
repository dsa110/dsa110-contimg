...
function initSocketIO(sockurl, pageid, id) {
  ...
  socket.on("disconnect", () => {
    ...
    if (socketActive) {
      obj.alert = true;
      obj.result =
        "The JS9 helper connection was unexpectedly severed (probably on the server side). Please try again.";
    } else if (connected) {
...
self.onmessage = function (e) {
  ...
  const uploadFITS = (fname, data, total, cur, left) => {
    ...
    if (left > 0) {
      ...
      // send data
      socket.emit("uploadfits", { cmd: `js9Xeq uploadfits ${fname}`, stdin: stdin }, (r) => {
        if (r.stdout === "OK") {
          // update progress bar
