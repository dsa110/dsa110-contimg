...
echo "🔄 Syncing API types from backend OpenAPI schema..."

# Ensure output directory exists
mkdir -p "$OUTPUT_DIR"

# Generate OpenAPI schema from backend
echo "📝 Extracting OpenAPI schema from backend..."
cd "$BACKEND_DIR"
...
if npx openapi-typescript "$OUTPUT_DIR/openapi.json" -o "$OUTPUT_DIR/api-types.ts" 2>/dev/null; then
    ...
    echo "⚠️  openapi-typescript not installed. Install with: npm install -D openapi-typescript"
