...
if [ ! -x "$CASA6_NODE" ]; then
  echo ":cross_mark: ERROR: casa6 Node.js not found at $CASA6_NODE" >&2
...
if [ "$CURRENT_NODE" != "$CASA6_NODE" ]; then
  echo ":cross_mark: ERROR: Not using casa6 Node.js" >&2
...
if [ "$(printf '%s\n' "$REQUIRED_VERSION" "$CURRENT_VERSION" | sort -V | head -n1)" != "$REQUIRED_VERSION" ]; then
  echo ":cross_mark: ERROR: Node.js version $CURRENT_VERSION < required $REQUIRED_VERSION" >&2
...
if lsof -ti:3210 > /dev/null 2>&1; then
  ...
  sleep 1
  echo ":check_mark: Port 3210 is now free"
fi

echo ":rocket: Starting Vite dev server on port 3210..."
