...
echo ":octagonal_sign: Stopping processes on ports: ${PORTS[*]}"
...
kill_port() {
    ...
    if [ -n "$pids" ]; then
        echo "   :check_mark: Killing process(es) on port $port (PIDs: $pids)"
        echo "$pids" | xargs -r kill -9 2>/dev/null || true
        sleep 1
        # Verify it's dead
        local remaining=$(lsof -ti:$port 2>/dev/null || true)
        if [ -n "$remaining" ]; then
            echo "   :warning_sign: Warning: Process still running, force killing..."
...
echo ""
echo ":spouting_whale: Restarting Docker services (if running)..."
...
if [ -f "docker-compose.yml" ]; then
    if command -v docker-compose &> /dev/null; then
        ...
        docker-compose up -d api dashboard-dev 2>/dev/null || {
            echo "   :warning_sign: Warning: Could not start Docker services (they may need to be built first)"
...
echo ""
echo ":rocket: Starting frontend dev server..."
...
if [ -f "/opt/miniforge/etc/profile.d/conda.sh" ]; then
    ...
    conda activate casa6 2>/dev/null || {
        echo "   :warning_sign: Warning: Could not activate casa6, continuing anyway..."
    }
fi
