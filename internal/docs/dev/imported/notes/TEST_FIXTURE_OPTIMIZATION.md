...
1. **Import bottleneck**: Importing `CalibratorMSGenerator` takes ~9.6 seconds
   (heavy CASA/astropy dependencies)
2. **Database initialization**: ~0.4 seconds (acceptable)
3. **Service initialization**: ~0.007 seconds (negligible)

## Root Cause

`MosaicOrchestrator.__init__` imports and initializes `CalibratorMSGenerator`
even when tests don't need it:
...
Mock the calibrator service initialization in the fixture since most tests don't
need the real service:
...
1. **Use `tmp_path` fixture** (already done) - faster than manual temp
   directories
