# Deep Study: DSA-110 Continuum Imaging Pipeline

## Executive Summary

The DSA-110 Continuum Imaging Pipeline (`dsa110-contimg`) is a production-grade radio astronomy data processing system designed for streaming, real-time processing of interferometric observations from the DSA-110 telescope array. The pipeline transforms raw UVH5 (HDF5) subband files into calibrated, imaged radio continuum products through a sophisticated multi-stage workflow with dependency resolution, retry policies, and comprehensive monitoring.
...
```
┌─────────────────┐
│  UVH5 Files     │  (16 subbands per group)
│  /data/incoming │
└────────┬────────┘
         │
         ▼
...
┌─────────────────────────────────────┐
│  Conversion Stage                   │
│  - UVH5 → MS (CASA Measurement Set) │
│  - Strategy orchestrator            │
│  - Parallel subband writers         │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────┐
│  Calibration Stage                  │
│  - Calibrator matching (VLA catalog)│
│  - Solve: K/BA/BP/GA/GP/2G          │
│  - Registry management              │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────┐
│  Apply Calibration Stage            │
│  - Apply active caltables           │
│  - Verify application               │
└────────┬────────────────────────────┘
         │
         ▼
...
### 6.1 Systemd Deployment (Production)
...
**Configuration**:
- Edit `ops/systemd/contimg.env` for environment variables
- Install units: `sudo cp ops/systemd/*.service /etc/systemd/system/`
- Enable: `sudo systemctl enable --now contimg-stream.service contimg-api.service`
...
**Configuration**:
- Copy `ops/docker/.env.example` to `ops/docker/.env`
- Set absolute paths for `REPO_ROOT`, `CONTIMG_*`, `UID`/`GID`
...
python -m dsa110_contimg.mosaic.cli build \
  --products-db state/products.sqlite3 \
  --name night_20240101 \
  --output /data/mosaics/night_20240101.img
```
...
- **Synthetic UVH5 Generation** (`simulation/make_synthetic_uvh5.py`):
  - PyUVSim-based simulation
  - Configurable sources and telescope layout
  - Validation tools

### 8.3 Error Handling

- **Error Context** (`utils/error_context.py`):
...
├── calibration/      # Calibration solving and application
├── catalog/          # Catalog utilities (NVSS, VLA)
├── conversion/       # UVH5 → MS conversion
├── database/         # Database schemas and helpers
├── imaging/          # Imaging (tclean/WSClean)
├── mosaic/           # Mosaic planning and building
├── photometry/       # Forced photometry
├── pipeline/         # Pipeline framework
├── pointing/         # Pointing monitoring
