...
CSV fallback has been disabled by default. All catalog queries now require
SQLite databases. CSV infrastructure remains available but must be explicitly
enabled via `use_csv_fallback=True` parameter.
...
1. **Egalitarian Approach**: All catalogs (NVSS, RAX, VLASS) now rely equally on
   SQLite databases
...
**No changes needed** - existing code continues to work, but will return empty
DataFrames if SQLite databases are unavailable.
...
- **Breaking Change**: Code that relied on automatic CSV fallback will now
  return empty DataFrames when SQLite databases are unavailable
- **Performance**: Encourages optimal performance by requiring SQLite databases
- **Consistency**: All catalogs now have equal treatment (SQLite-first, CSV
  opt-in)
...
- [Catalog Logging Migration](catalog_logging_migration.md) - Logging
  improvements for fallback notifications
