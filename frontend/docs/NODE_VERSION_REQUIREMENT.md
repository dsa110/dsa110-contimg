...
- **System Node.js v16.20.2** is incompatible with Vitest/Vite crypto requirements
- **Casa6 Node.js v22.6.0** provides modern JavaScript features and crypto support
- Tests will fail with `crypto$2.getRandomValues is not a function` if using system Node.js

## Safeguards

### 1. Pre-flight Check Script
`scripts/check-casa6-node.sh` automatically verifies casa6 Node.js before running tests.
...
`scripts/lib/error-detection.sh` detects frontend test context and enforces casa6 Node.js.
