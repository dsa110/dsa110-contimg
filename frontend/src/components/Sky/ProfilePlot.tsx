...
export default function ProfilePlot({ profileData, height = 400, onExportPNG }: ProfilePlotProps & { onExportPNG?: () => void }) {
  const { plotData, layout, fitInfo } = useMemo(() => {
    ...
    const plotLayout: Partial<Layout> = {
      title: `${profileData.profile_type.charAt(0).toUpperCase() + profileData.profile_type.slice(1)} Profile` as any,
      xaxis: {
  ...
  if (!profileData) {
    return (
      <Paper sx={{ p: 2 }}>
        ...
        <Alert severity="info">No profile data available. Draw a profile on the image to extract data.</Alert>
      </Paper>
    );
  }
