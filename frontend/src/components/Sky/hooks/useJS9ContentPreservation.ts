...
export function useJS9ContentPreservation({
  ...
  useLayoutEffect(() => {
    if (!containerRef.current || !initialized || !isJS9Ready || !imageLoadedRef.current || loading) return;
  ...
  }, [displayId, initialized, isJS9Ready, loading, imagePath, imageLoadedRef, getDisplaySafe, containerRef]);
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      // Only restore if we're not currently loading a new image
      if (div.children.length === 0 && imageLoadedRef.current && !loading && js9Service.isAvailable() && imagePath) {
        const display = getDisplaySafe(displayId);
