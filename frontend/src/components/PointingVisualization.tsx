  ...
  const { plotData, layout } = useMemo(() => {
    ...
    if (currentPointing) {
      data.push({
        ...
        hovertemplate:
          '<b>Current Pointing</b><br>RA: %{x:.4f}°<br>Dec: %{y:.4f}°<extra></extra>',
      });
    }
    ...
    <Paper sx={{ p: 2 }}>
      ...
      {!isHealthy && (
        <Alert severity="error" sx={{ mb: 2 }}>
          Pointing monitor health check failed: {monitorStatus?.issues?.join(', ') || 'Unknown error'}
        </Alert>
      )}
                ...
                <>
                  <Box>
                    ...
                    <Typography variant="body2">
                      {monitorStatus.metrics.files_processed}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">
