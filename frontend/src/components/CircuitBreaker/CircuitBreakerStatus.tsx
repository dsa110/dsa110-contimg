              ...
              <CardContent>
                <Stack spacing={2}>
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Box sx={{ flexGrow: 1 }}>
                      <Typography variant="h6">{breaker.name.replace('_', ' ').toUpperCase()}</Typography>
                      <Chip
                  ...
                  <Box>
                    ...
                    {breaker.last_failure_time && (
                      <Typography variant="body2" color="text.secondary">
                        Last Failure:{' '}
                        {new Date(breaker.last_failure_time * 1000).toLocaleString()}
                      </Typography>
                    )}
                    <Typography variant="body2" color="text.secondary">
      ...
      <Dialog open={resetDialogOpen} onClose={() => setResetDialogOpen(false)}>
        ...
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 2 }}>
            ...
            <strong>{selectedBreaker}</strong>? This will close the circuit and allow
            requests to flow through again.
          </Alert>
        </DialogContent>
        <DialogActions>
