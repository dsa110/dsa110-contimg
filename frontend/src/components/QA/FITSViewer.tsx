...
export default function FITSViewer({
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((error) => {
        setViewerError(error.response?.data?.detail || error.message || 'Failed to load FITS viewer');
        setLoadingViewer(false);
      });
    ...
    <Paper sx={{ p: 2, bgcolor: 'background.paper', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {infoError && (
        <Alert severity="error" sx={{ mb: 2 }}>
          Error loading FITS info: {infoError instanceof Error ? infoError.message : 'Unknown error'}
        </Alert>
      )}
