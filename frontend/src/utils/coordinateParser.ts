...
export function parseRA(input: string): number | null {
  ...
  const hmsShortMatch = trimmed.match(
    /^(\d{1,2})(?:h|:|\s)\s*(\d{1,2})(?:m)?$/i
  );
...
export function parseDec(input: string): number | null {
  ...
  const dmsShortMatch = working.match(
    /^(\d{1,2})(?:d|°|:|\s)\s*(\d{1,2})(?:m|')?$/i
  );
 ...
 */
export function validateCoordinates(ra: number | null, dec: number | null): {
  valid: boolean;
  raError?: string;
  decError?: string;
} {
