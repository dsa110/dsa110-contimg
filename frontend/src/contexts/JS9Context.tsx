...
export function JS9Provider({ children }: JS9ProviderProps) {
  ...
  const hasImage = useCallback((displayId: string) => {
    const display = getDisplay(displayId);
    return !!(display?.im);
  }, [getDisplay]);

  // Get display state
  const getDisplayState = useCallback((displayId: string): DisplayState | null => {
  ...
  const updateDisplayState = useCallback((displayId: string) => {
    const display = getDisplay(displayId);
    const imageId = getImageId(displayId);
    const hasImg = hasImage(displayId);
  ...
  const loadImage = useCallback(async (displayId: string, imagePath: string): Promise<void> => {
    if (!isJS9Available()) {
  ...
  const refreshDisplay = useCallback((displayId: string) => {
    updateDisplayState(displayId);
  }, [updateDisplayState]);
