...
function normalizeStatus(status: ServiceStatusType): "healthy" | "degraded" | "unhealthy" | "unknown" {
  switch (status) {
    case "running":
    case "healthy":
      return "healthy";
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
      <div className="flex items-center gap-3">
        <span className={`w-3 h-3 rounded-full ${statusColors[displayStatus]}`} />
...
function SystemHealthPanel() {
  ...
  const overallStatusColor = {
    ...
    degraded: "text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30",
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <div className="text-2xl font-bold text-red-600 dark:text-red-400">
              {data.summary.unhealthy ?? data.summary.error ?? data.summary.stopped ?? 0}
            </div>
      ...
      {activeAlerts.length === 0 ? (
        ...
        <div className="space-y-2">
          {activeAlerts.map((alert) => (
            <div
              key={alert.id}
              className={`p-3 rounded-lg border-l-4 ${alertColors[alert.severity]}`}
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    {alert.source || alert.alert_type} • {new Date(alert.created_at || alert.triggered_at).toLocaleString()}
                  </div>
                </div>
                <span
