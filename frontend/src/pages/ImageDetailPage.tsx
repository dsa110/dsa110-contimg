...
export default function ImageDetailPage() {
  ...
  // Fetch image data
  const { data: image, isLoading: imageLoading, error: imageError } = useImageDetail(
    imageId ? parseInt(imageId, 10) : null
  );
    ...
    return (
      <Container maxWidth="xl" sx={{ py: 4 }}>
        <Alert severity="error">
          Failed to load image details. {imageError instanceof Error ? imageError.message : 'Unknown error'}
        </Alert>
      </Container>
    );
  }
            ...
            <CardContent>
              ...
              {image.ra !== null && image.dec !== null && (
                <>
                  <Typography variant="body2" paragraph>
                    ...
                    {image.ra_hms || formatRA(image.ra)} {image.dec_dms || formatDec(image.dec)}<br />
                    <strong>Decimal:</strong> {image.ra.toFixed(6)} {image.dec.toFixed(6)}
                  </Typography>
                </>
              )}
              
              {image.l !== undefined && image.l !== null && image.b !== undefined && image.b !== null && (
                <Typography variant="body2" paragraph>
              ...
              {(image.beam_bmaj || image.beam_bmin || image.beam_bpa) && (
                <>
                  ...
                  <Typography variant="body2" paragraph>
                    ...
                    {image.beam_bmaj && `BMAJ: ${(image.beam_bmaj * 3600).toFixed(3)}&Prime;`}<br />
                    {image.beam_bmin && `BMIN: ${(image.beam_bmin * 3600).toFixed(3)}&Prime;`}<br />
                    {image.beam_bpa !== null && image.beam_bpa !== undefined && `BPA: ${image.beam_bpa.toFixed(2)}&deg;`}
                  </Typography>
                </>
              )}
              ...
              {image.rms_median !== null && image.rms_median !== undefined && (
                <>
                  ...
                  <Typography variant="body2" paragraph>
                    ...
                    Median: {image.rms_median.toFixed(3)} mJy<br />
                    {image.rms_min !== null && image.rms_min !== undefined && `Min: ${image.rms_min.toFixed(3)} mJy`}<br />
                    {image.rms_max !== null && image.rms_max !== undefined && `Max: ${image.rms_max.toFixed(3)} mJy`}
                  </Typography>
                </>
              )}
              ...
              {(image.frequency || image.bandwidth) && (
                <>
                  <Divider sx={{ my: 2 }} />
                  <Typography variant="body2" paragraph>
                    {image.frequency && <><strong>Frequency:</strong> {image.frequency.toFixed(1)} MHz<br /></>}
                    {image.bandwidth && <><strong>Bandwidth:</strong> {image.bandwidth.toFixed(1)} MHz</>}
                  </Typography>
                </>
              )}
              ...
              <Typography variant="body2" paragraph>
                <strong>Measurements:</strong> {image.n_meas.toLocaleString()}<br />
