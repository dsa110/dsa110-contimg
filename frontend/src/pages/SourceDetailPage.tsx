...
export default function SourceDetailPage() {
  ...
  // Fetch source data
  const { data: source, isLoading: sourceLoading, error: sourceError } = useSourceDetail(sourceId || null);
    ...
    return (
      <Container maxWidth="xl" sx={{ py: 4 }}>
        <Alert severity="error">
          Failed to load source details. {sourceError instanceof Error ? sourceError.message : 'Unknown error'}
        </Alert>
      </Container>
    );
  }
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Typography variant="body2" paragraph>
                ...
                {formatRA(source.ra_deg)} {formatDec(source.dec_deg)}<br />
              ...
              <Typography variant="body2" paragraph>
                <strong>Flux Statistics:</strong><br />
                {source.mean_flux_jy && (
                  <>
                    Mean Flux: {(source.mean_flux_jy * 1000).toFixed(3)} mJy<br />
                  </>
                )}
                {source.std_flux_jy && (
                  <>
                    Std Flux: {(source.std_flux_jy * 1000).toFixed(3)} mJy<br />
                  </>
                )}
                {source.max_snr && (
                  <>
                    Max SNR: {source.max_snr.toFixed(2)}<br />
                  </>
                )}
                {source.variability_metrics && (
                  <>
                    Variability (v): {source.variability_metrics.v.toFixed(3)}<br />
