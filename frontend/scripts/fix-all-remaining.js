...
const fixers = {
  ...
  TS2322: (error, filePath, lines) => {
    ...
    if (error.message.includes("UseQueryResult")) {
      const newLine = line.replace(/return useQuery\(/, "return useQuery(");
    ...
    if (error.message.includes("is not assignable to type")) {
      ...
      if (valueMatch) {
        ...
        if (typeMatch) {
          ...
          const newLine = line.replace(valueMatch[3], `${valueMatch[3]} as ${expectedType}`);
          return { line: error.line - 1, newLine };
        }
      }
    }
  ...
  TS18048: (error, filePath, lines) => {
    ...
    if (propMatch) {
      ...
      const newLine = line.replace(new RegExp(`\\b${propName}\\.`, "g"), `${propName}?.`);
      if (newLine !== line) {
        return { line: error.line - 1, newLine };
      }
    }
