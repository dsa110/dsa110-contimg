...
function fixMissingExports(errors) {
  ...
  errors.forEach(err => {
    if (err.message.includes('DataInstanceList')) {
      ...
      if (fs.existsSync(typesPath)) {
        const content = fs.readFileSync(typesPath, 'utf8');
        if (!content.includes('export interface HealthSummary') && !content.includes('export type HealthSummary')) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (lineIndex >= 0 && lineIndex < lines.length) {
          ...
          if (line.includes('modified_time')) {
            ...
            fixes.push({ file: err.file, line: err.line, action: 'Replaced modified_time with modified_at' });
          }
        }
      }
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (lineIndex >= 0 && lineIndex < lines.length) {
          ...
          if (line.includes('as CARTAMessageType')) {
            ...
            lines[lineIndex] = line.replace(/as CARTAMessageType/g, 'as unknown as CARTAMessageType');
            fs.writeFileSync(filePath, lines.join('\n'), 'utf8');
            fixes.push({ file: err.file, line: err.line, action: 'Fixed CARTAMessageType conversion' });
          }
        }
      }
    }
...
function main() {
  ...
  console.log(`  - Type mismatches (TS2322/TS2345): ${categories.typeMismatches.length} - May need manual fixes`);
  console.log(`  - Overload errors (TS2769): ${categories.overloadErrors.length} - May need manual fixes`);
  console.log(`  - Export errors (TS2459): ${categories.exportErrors.length} - May need manual fixes`);
  console.log(`  - Index errors (TS2538): ${categories.indexErrors.length} - May need manual fixes`);
}
