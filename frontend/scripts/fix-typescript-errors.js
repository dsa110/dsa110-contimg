...
const fixPatterns = [
  {
    ...
    messageMatch: /Type 'RefObject<HTMLDivElement \| null>' is not assignable to type 'RefObject<HTMLDivElement>'/,
    fix: (line, context) => {
      // Find type annotations and remove | null
      if (line.includes('RefObject<HTMLDivElement | null>')) {
        return line.replace(
          /RefObject<HTMLDivElement \| null>/g,
  ...
  },
  {
    ...
    fix: (line, context) => {
      ...
      if (funcCallMatch) {
        ...
        if (lastArg && !lastArg.includes('parseInt') && !lastArg.includes('Number') && !lastArg.includes('+')) {
  ...
  },
  {
    ...
    messageMatch: /Type '(JobParams|ConversionJobParams)' is not assignable to type 'Record<string, unknown>'/,
    fix: (line, context) => {
      // Add type assertion
      if (line.includes('params:') && (line.includes('JobParams') || line.includes('ConversionJobParams'))) {
        return line.replace(
          /(params:\s*)(\w+)/,
          '$1$2 as Record<string, unknown>'
        );
      }
      return null;
    }
  ...
  },
  {
    ...
    messageMatch: /Type 'number \| null \| undefined' is not assignable to type 'number \| undefined'/,
    fix: (line, context) => {
  ...
  },
  {
    ...
    messageMatch: /Argument of type 'string \| undefined' is not assignable to parameter of type 'string'/,
    fix: (line, context) => {
  ...
  },
  {
    ...
    messageMatch: /Type 'MSListEntry\[\] \| undefined' is not assignable to type 'number \| undefined'/,
    fix: (line, context) => {
      ...
      for (const checkLine of linesToCheck) {
        ...
        if (propMatch) {
          ...
          if ((varName.includes('List') || varName.includes('items') || varName.includes('entries') || varName.includes('filtered')) 
  ...
  {
    name: 'number | null | undefined to number | undefined',
    messageMatch: /Argument of type 'number \| null \| undefined' is not assignable to parameter of type 'number \| undefined'/,
    fix: (line, context) => {
      // Add nullish coalescing to remove null
      const funcCallMatch = line.match(/(\w+)\(([^)]+)\)/);
      if (funcCallMatch) {
  ...
  },
  {
    ...
    fix: (line, context) => {
      ...
      if (line.includes('return') && (line.includes('useQuery') || context.prev.includes('useQuery'))) {
          ...
          if (i >= 0 && i < context.allLines.length) {
            ...
            if (funcMatch) {
              ...
              if (returnMatch && !line.includes('as UseQueryResult')) {
                return line.replace(
                  /(return\s+)(.+)/,
                  `$1$2 as UseQueryResult<${expectedType}>`
                );
              }
            }
          }
  ...
  },
  {
    ...
    messageMatch: /Argument of type '\(prev: Record<string, .*>\) => .*' is not assignable to parameter of type 'SetStateAction<Record<string, .*>>'/,
    fix: (line, context) => {
  ...
  },
  {
    ...
    messageMatch: /Argument of type '".*"' is not assignable to parameter of type 'CARTAMessageType'/,
    fix: (line, context) => {
  ...
  },
  {
    ...
    messageMatch: /Type 'ValidationRule<number>' is not assignable to type 'ValidationRule<string>'/,
    fix: (line, context) => {
