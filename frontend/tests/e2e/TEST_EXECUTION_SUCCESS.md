...
**Problem:** Tests were failing with `ERR_CONNECTION_REFUSED` because they were
...
**Problem:** Playwright was trying to start its own web server, causing
conflicts with Docker Compose.
...
**Solution:** Browsers are already installed in the Docker container. If this
error appears, ensure you're running tests inside the container, not on the
host.
...
- [Implementation Summary](./IMPLEMENTATION_SUMMARY.md) - Test infrastructure
  details
