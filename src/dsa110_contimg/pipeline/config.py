...
class ConversionConfig(BaseModel):
    ...
    writer: str = Field(
        default="auto", description="Writer strategy: 'auto', 'parallel-subband', or 'pyuvdata'")
...
class ImagingConfig(BaseModel):
    ...
    run_catalog_validation: bool = Field(
        default=True, description="Run catalog-based flux scale validation after imaging")
    ...
    mask_radius_arcsec: float = Field(
        default=60.0, ge=10.0, le=300.0, description="Mask radius around NVSS sources (arcsec)")
...
class ValidationConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Enable validation stage")
    catalog: str = Field(
        default="nvss", description="Catalog to use for validation ('nvss' or 'vlass')")
...
class PipelineConfig(BaseModel):
    ...
    def from_env(cls, validate_paths: bool = True, required_disk_gb: float = 50.0) -> PipelineConfig:
        ...
        def safe_int(env_var: str, default: str, min_val: Optional[int] = None, max_val: Optional[int] = None) -> int:
        ...
        def safe_float(env_var: str, default: str, min_val: Optional[float] = None, max_val: Optional[float] = None) -> float:
        ...
        config = cls(
            ...
            conversion=ConversionConfig(
                ...
                max_workers=safe_int("PIPELINE_MAX_WORKERS",
                                     "4", min_val=1, max_val=32),
                stage_to_tmpfs=os.getenv(
                    "PIPELINE_STAGE_TO_TMPFS", "true").lower() == "true",
            ...
            calibration=CalibrationConfig(
                ...
                auto_select_refant=os.getenv(
                    "PIPELINE_AUTO_SELECT_REFANT", "true").lower() == "true",
            ),
            imaging=ImagingConfig(
                ...
                use_nvss_mask=os.getenv(
                    "PIPELINE_USE_NVSS_MASK", "true").lower() == "true",
