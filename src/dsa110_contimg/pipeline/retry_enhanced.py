...
def retry_with_backoff(
    ...
    if TENACITY_AVAILABLE:
        ...
        return retry(stop=stop, wait=wait, retry=retry_condition, reraise=True)
    else:
...
def retry_on_database_error(
    max_attempts: int = 3, initial_delay: float = 0.5, max_delay: float = 10.0
) -> Callable[[Callable[..., T]], Callable[..., T]]:
...
retry_calibration_solve = retry_on_transient_error(
    max_attempts=2, initial_delay=5.0, max_delay=300.0
)
