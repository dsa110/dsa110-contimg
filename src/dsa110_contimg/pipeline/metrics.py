...
if PROMETHEUS_AVAILABLE:
    ...
    pipeline_throughput = Gauge(
        'pipeline_throughput_items_per_hour',
        'Pipeline throughput',
        ['stage_name']
    )
    ...
    ese_detection_requests = SimpleCounter('ese_detection_requests_total', 'Total ESE detection requests')
    ese_detection_duration = SimpleHistogram('ese_detection_duration_seconds', 'ESE detection execution time')
    ese_candidates_detected = SimpleGauge('ese_candidates_current', 'Current number of ESE candidates')
    variability_stats_computation_time = SimpleHistogram('variability_stats_computation_seconds', 'Time to compute variability statistics')
    calibration_solve_attempts = SimpleCounter('calibration_solve_attempts_total', 'Total calibration solve attempts')
    calibration_solve_duration = SimpleHistogram('calibration_solve_duration_seconds', 'Calibration solve execution time')
    calibration_table_age_hours = SimpleGauge('calibration_table_age_hours', 'Age of calibration tables in hours')
    calibration_quality_score = SimpleGauge('calibration_quality_score', 'Quality score of calibration solution')
    photometry_measurements_total = SimpleCounter('photometry_measurements_total', 'Total photometry measurements')
    photometry_measurement_duration = SimpleHistogram('photometry_measurement_duration_seconds', 'Photometry measurement execution time')
    photometry_normalization_requests = SimpleCounter('photometry_normalization_requests_total', 'Total normalization requests')
    normalization_correction_factor = SimpleHistogram('normalization_correction_factor', 'Distribution of correction factors')
    normalization_reference_count = SimpleHistogram('normalization_reference_count', 'Number of reference sources used')
    pipeline_stage_duration = SimpleHistogram('pipeline_stage_duration_seconds', 'Duration of pipeline stages')
    pipeline_stage_attempts = SimpleCounter('pipeline_stage_attempts_total', 'Total pipeline stage attempts')
...
def record_ese_detection(duration: float, candidates: int, source: str = 'all', min_sigma: float = 5.0):
...
def record_calibration_solve(duration: float, calibrator_name: str, calibration_type: str, status: str):
    ...
    if PROMETHEUS_AVAILABLE:
        ...
        if status == 'success':
            calibration_solve_duration.observe(duration, {
                'calibrator_name': calibrator_name,
                'calibration_type': calibration_type
            })
