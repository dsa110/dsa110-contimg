...
def check_disk_space(path: Path, required_gb: float) -> Tuple[bool, str]:
    ...
    try:
        ...
        if has_space:
            return True, f"Sufficient disk space: {free_gb:.1f}GB available (need {required_gb:.1f}GB)"
        else:
            return False, f"Insufficient disk space: {free_gb:.1f}GB available (need {required_gb:.1f}GB)"
...
def check_system_resources(
    ...
    try:
        ...
        if mem_gb < min_memory_gb:
            issues.append(f"Insufficient memory: {mem_gb:.1f}GB available (need {min_memory_gb:.1f}GB)")
    except ImportError:
