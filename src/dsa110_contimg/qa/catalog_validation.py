...
class CatalogValidationResult:
    ...
    completeness_limit_jy: Optional[float] = None  # Flux density at which completeness drops below threshold
                        ...
                        for dy in [-1, 0, 1]:
                            for dx in [-1, 0, 1]:
                                if dy == 0 and dx == 0:
                                    continue
                                ny, nx = y + dy, x + dx
                                if (0 <= ny < data.shape[0] and 0 <= nx < data.shape[1] and
                                    (ny, nx) not in visited and above_threshold[ny, nx]):
                                    stack.append((ny, nx))
...
def validate_astrometry(
    ...
    if max_offset > max_offset_arcsec:
        issues.append(f"Maximum astrometric offset ({max_offset:.2f} arcsec) exceeds threshold ({max_offset_arcsec} arcsec)")
    ...
    if n_matched < n_detected * 0.3:
        warnings.append(f"Only {n_matched}/{n_detected} detected sources matched to catalog")
    
    matched_pairs = [
        (
            (det.ra.deg, det.dec.deg),
            (cat.ra.deg, cat.dec.deg),
            sep.value
        )
...
def validate_flux_scale(
    ...
    for idx, cat_row in catalog_sources.iterrows():
        ...
        try:
            ...
            logger.warning(f"Failed forced photometry for source at ({catalog_ra:.4f}, {catalog_dec:.4f}): {e}")
            n_failed += 1
            continue
...
def validate_source_counts(
    ...
    if n_detected > 0 and len(detected_sources) > 0:
        ...
        if len(matched_catalog_indices) > 0:
            catalog_filtered.iloc[matched_catalog_indices, catalog_filtered.columns.get_loc("detected")] = True
    ...
    catalog_filtered["flux_bin"] = pd.cut(
        catalog_filtered["flux_jy"],
        bins=flux_bins,
        include_lowest=True
    )
    
    # Calculate completeness per bin
    bin_stats = catalog_filtered.groupby("flux_bin").agg({
    ...
    if n_matched < 3:
        warnings.append(f"Low number of matched sources ({n_matched}), completeness estimate may be unreliable")
...
) -> Tuple[Optional[CatalogValidationResult], Optional[CatalogValidationResult], Optional[CatalogValidationResult]]:
    """
