...
def validate_mosaic_quality(
    ...
    try:
        ...
        passed = (
            max_seam_deviation <= config.max_seam_flux_deviation
            and max_wcs_offset <= config.max_wcs_offset_arcsec
            and min_overlap_fraction >= config.min_overlap_fraction
            and max_noise_ratio <= config.max_noise_ratio
        )
