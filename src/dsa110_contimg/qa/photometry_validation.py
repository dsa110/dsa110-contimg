    ...
    try:
        ...
        if len(matched_results) == 0:
            return PhotometryValidationResult(
                ...
                details={"n_photometry": len(photometry_results), "n_catalog": len(catalog_sources or [])},
            )
        ...
        for match in matched_results:
            ...
            source_passed = (
                flux_error_frac <= config.max_flux_error_fraction and
                position_offset <= config.max_position_offset_arcsec
            )
    ...
    for phot_result in photometry_results:
        ...
        if best_match:
            matched.append({
                ...
                "source_id": best_match.get("source_id", f"{best_match['ra']}_{best_match['dec']}"),
            })
