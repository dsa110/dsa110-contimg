...
def validate_variability_detection(
    ...
    if len(photometry_history) < config.min_observations:
        return VariabilityValidationResult(
            ...
            details={
                "n_observations": len(photometry_history),
                "min_required": config.min_observations,
            },
            errors=[
                f"Need at least {config.min_observations} observations for variability validation"
            ],
        )
    ...
    try:
        ...
        chi_squared_error = (
            abs(chi_squared - expected_chi_squared) / expected_chi_squared
            if expected_chi_squared > 0
            else 0.0
        )

        # Determine if variability is real (not systematic error)
        is_real_variability = (
            chi_squared >= config.min_chi_squared
            and variability_fraction >= config.min_variability_fraction
            and chi_squared_error < 0.1  # Chi-squared calculated correctly
        )
