...
def create_cutout(
    ...
    try:
        ...
        raise ValueError(
            f"Failed to create cutout from {fits_path}: {e}") from e
        ...
        for source in overlay_sources:
            if 'ra' in source and 'dec' in source:
                ...
                if 0 <= pix_coords[0] < data.shape[1] and 0 <= pix_coords[1] < data.shape[0]:
                    ...
                    if 'label' in source:
                        ax.text(pix_coords[0] + 5, pix_coords[1] + 5, source['label'],
    ...
    if overlay_catalog:
        try:
            ...
            center_coord = SkyCoord(
                wcs.wcs.crval[0], wcs.wcs.crval[1], unit='deg')
            ...
            for _, row in catalog_sources.iterrows():
                ...
                if 0 <= pix_coords[0] < data.shape[1] and 0 <= pix_coords[1] < data.shape[0]:
                    ax.plot(pix_coords[0], pix_coords[1],
                            'gx', markersize=8, markeredgewidth=1.5)
        except Exception as e:
