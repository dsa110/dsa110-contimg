...
def cmd_image(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.validate_flux_scale:
            ...
            qa_output_dir = (
                Path(args.output_dir) if args.output_dir else image_path.parent / "qa"
            )
            qa_output_dir.mkdir(parents=True, exist_ok=True)
...
def cmd_mosaic(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT path FROM mosaics WHERE id = ?", (mosaic_id,)
        )
        row = cursor.fetchone()
        if not row:
        ...
        qa_output_dir = (
            Path(args.output_dir) if args.output_dir else mosaic_path.parent / "qa"
        )
        qa_output_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            results["tile_quality"] = tile_qa.model_dump() if hasattr(tile_qa, "model_dump") else str(tile_qa)
        except Exception as e:
        ...
        try:
            ...
            results["tiles_consistency"] = consistency.model_dump() if hasattr(consistency, "model_dump") else str(consistency)
        except Exception as e:
        ...
        if args.check_astrometry:
            ...
            try:
                ...
                results["astrometry"] = astro.model_dump() if hasattr(astro, "model_dump") else str(astro)
            except Exception as e:
        ...
        if args.check_calibration:
            ...
            try:
                ...
                results["calibration_consistency"] = cal_consistency.model_dump() if hasattr(cal_consistency, "model_dump") else str(cal_consistency)
            except Exception as e:
        ...
        if args.check_primary_beam:
            ...
            try:
                ...
                results["primary_beam_consistency"] = pb_consistency.model_dump() if hasattr(pb_consistency, "model_dump") else str(pb_consistency)
            except Exception as e:
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        qa_output_dir = Path(args.output_dir) if args.output_dir else Path(record.stage_path).parent / "qa"
        qa_output_dir.mkdir(parents=True, exist_ok=True)
...
def main(argv: list = None) -> int:
    ...
    cal_parser.add_argument(
        "--output-dir", type=str, help="Output directory for QA results"
    )
    cal_parser.add_argument(
        "--check-caltables",
        action="store_true",
    ...
    img_parser.add_argument(
        "--catalog", type=str, default="nvss", help="Catalog for validation"
    )
    img_parser.add_argument(
    ...
    mosaic_parser.add_argument(
        "--products-db", type=str, help="Path to products database"
    )
    mosaic_parser.add_argument(
        "--output-dir", type=str, help="Output directory for QA results"
    )
    ...
    report_parser.add_argument(
        "--min-flux", type=float, default=0.01, help="Minimum flux (Jy)"
    )
    report_parser.add_argument(
        "--max-flux", type=float, default=10.0, help="Maximum flux (Jy)"
    )
    report_parser.add_argument(
        "--max-flux-error",
        type=float,
