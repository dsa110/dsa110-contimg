    ...
    try:
        with table(ms_path, readonly=True, ack=False) as tb:
            ...
            if data_column_present and n_rows > 0:
                ...
                try:
                    data_sample = tb.getcol(check_data_column, startrow=indices[0], nrow=len(indices))
                    ...
                    if len(unflagged_amps) > 0:
                        ...
                        if len(nonzero_amps) > 0:
                            ...
                            amplitude_range = (float(np.min(nonzero_amps)), float(np.max(nonzero_amps)))
                        else:
            ...
            if median_amplitude > 0 and (amplitude_range[1] / amplitude_range[0] > 1000):
                warnings.append(f"Very large amplitude dynamic range: {amplitude_range[1]/amplitude_range[0]:.1f}x")
