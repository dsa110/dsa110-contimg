...
def create_synthetic_fits(
    ...
    Args:
        ...
        n_sources: Number of point sources to add (if sources not provided)
        source_flux_range_jy: (min, max) flux range for sources in Jy
        beam_fwhm_pix: Beam FWHM in pixels
        sources: Optional list of dicts with keys: ra_deg, dec_deg, flux_jy, name
                 If provided, n_sources is ignored
        mark_synthetic: If True, add synthetic provenance markers to header
