...
def query_caltables_by_source(
    registry_db: Path, ms_path: str
) -> List[CalTable]:
...
def impact_analysis(
    registry_db: Path, caltable_paths: List[str]
) -> List[str]:
...
def get_caltable_provenance(
    registry_db: Path, caltable_path: str
) -> Optional[CalTable]:
