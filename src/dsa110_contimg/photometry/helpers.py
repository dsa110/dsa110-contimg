    ...
    with fits.open(fits_path) as hdul:
            ...
            if wcs.has_celestial:
                ...
                if naxis1 > 0 and naxis2 > 0:
                    ...
                    if hdr.get("NAXIS", 0) >= 2:
                        ...
                        logger.debug(
                            f"Extracted field center from WCS: RA={ra:.6f}, Dec={dec:.6f}"
                        )
        ...
        try:
            ...
            if ra is not None and dec is not None:
                logger.debug(
                    f"Extracted field center from CRVAL: RA={ra:.6f}, Dec={dec:.6f}"
                )
                return (float(ra), float(dec))
...
def query_sources_for_mosaic(
    ...
    try:
        ...
        logger.error(
            f"Failed to query sources for mosaic {mosaic_path}: {e}", exc_info=True
        )
        return []
