...
def detect_ese_candidates(
    ...
    try:
        ...
        if not rows:
            logger.info(
                f"No sources found with sigma_deviation >= {min_sigma}")
            return []
        ...
        for row in rows:
            ...
            if existing:
                ...
                if significance > min_sigma:
                    ...
                    logger.debug(
                        f"Skipping {source_id_val} - already flagged as active candidate"
                    )
                    continue
            else:
                ...
                logger.info(
                    f"Flagged ESE candidate {source_id_val} "
                    f"(significance: {significance:.2f})"
                )
...
def _recompute_variability_stats(conn: sqlite3.Connection) -> None:
    ...
    for (source_id,) in sources:
        ...
        # Compute statistics
        fluxes = [float(r["peak_jyb"])
                  for r in rows if r["peak_jyb"] is not None]
        if len(fluxes) < 2:
            continue
        ...
        if len(errors) == len(fluxes) and all(e > 0 for e in errors):
            chi2 = np.sum(((np.array(fluxes) - mean_flux)
                          ** 2) / (np.array(errors) ** 2))
        ...
        last_measured_at = float(
            last_row["measured_at"]) if last_row["measured_at"] else time.time()
