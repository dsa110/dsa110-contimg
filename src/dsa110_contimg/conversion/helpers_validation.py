                    ...
                    with table(ms_path, readonly=True, ack=False) as main_table:
                        if main_table.nrows() > 0:
                            ...
                            if times.size > 0:
                                ...
                                if (max_separation_arcsec > 60.0 and  # More than 1 arcmin
                                    max_separation_arcsec < expected_lst_change_arcsec * 1.2 and
                                    max_separation_arcsec > expected_lst_change_arcsec * 0.8):
                                    logger.info(
    ...
    try:
        with table(ms_path, readonly=True) as tb:
            ...
            if refant_list:
                ...
                refant_scores = {ant_id: ant_scores.get(ant_id, 0.0) for ant_id in refant_list if ant_id in unique_ants}
                if refant_scores:
                    best_ant_id = max(refant_scores, key=refant_scores.get)
                    best_ant_name = ant_names[best_ant_id]
                    logger.info(f"Selected reference antenna from provided list: {best_ant_name} (score: {ant_scores[best_ant_id]:.2f})")
            ...
            best_ant_name = ant_names[best_ant_id]
            logger.info(f"Selected best reference antenna: {best_ant_name} (score: {ant_scores[best_ant_id]:.2f})")
            return best_ant_name
