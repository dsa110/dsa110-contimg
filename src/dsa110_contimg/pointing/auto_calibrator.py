...
def get_data_time_range(products_db_path: Path) -> Optional[Tuple[Time, Time]]:
    ...
    try:
        ...
        logger.warning(
            f"Could not determine data time range: {e}", exc_info=True)
        return None


def check_bp_calibrator_registered(
    products_db: sqlite3.Connection, dec_deg: float, dec_tolerance_deg: float = DEC_TOLERANCE_DEG
    ...
    cursor.execute(
        ...
        WHERE status = 'active'
        AND dec_range_min <= ? AND dec_range_max >= ?
        """,
        (dec_deg + dec_tolerance_deg, dec_deg - dec_tolerance_deg),
    )

    result = cursor.fetchone()

    if result:
        return result  # (calibrator_name, ra_deg, dec_deg)
...
def find_brightest_vla_calibrator(
    ...
    try:
        ...
        filtered = vla_df[
            (vla_df['dec_deg'] >= dec_min) & (vla_df['dec_deg'] <= dec_max)
        ]
        ...
        if flux_col is None:
            ...
            logger.warning(
                "No flux column found in VLA catalog, using first match")
            row = filtered.iloc[0]
            return (
        ...
        filtered = filtered.sort_values(
            flux_col, ascending=False, na_last=True)
        row = filtered.iloc[0]
...
def find_brightest_nvss_source(
    ...
    try:
        ...
        if nvss_df.empty:
            logger.info(
                f"No NVSS sources found within ±{dec_tolerance_deg}° of Dec={dec_deg:.2f}°")
            return None
        ...
        filtered = nvss_df[
            (nvss_df['dec_deg'] >= dec_min) & (nvss_df['dec_deg'] <= dec_max)
        ]

        if filtered.empty:
            logger.info(f"No NVSS sources within declination tolerance")
            return None
        ...
        if flux_col is None:
            logger.warning(
                "No flux column found in NVSS catalog, using first match")
            row = filtered.iloc[0]
...
def register_and_precalculate_transits(
    ...
    try:
        ...
        logger.info(
            f"Registered BP calibrator {calibrator_name} "
        ...
        if time_range:
            ...
            days_forward = int((end_time - Time.now()).to(u.day).value) + 1
            ...
            max_days_back = 60
            logger.warning(
                "Could not determine data time range, using default 60 days")
        ...
        logger.error(
            f"Failed to register and pre-calculate transits: {e}", exc_info=True)
        return False
...
def handle_declination_change(
    ...
    try:
        ...
        registered = check_bp_calibrator_registered(
            products_db, new_dec_deg, DEC_TOLERANCE_DEG
        )

        if registered:
            calibrator_name, ra_deg, dec_deg = registered
            logger.info(
                f"BP calibrator {calibrator_name} already registered "
                f"for Dec={new_dec_deg:.2f}°"
            )
        ...
        vla_result = find_brightest_vla_calibrator(
            new_dec_deg, DEC_TOLERANCE_DEG)
        ...
        nvss_result = find_brightest_nvss_source(
            new_dec_deg, DEC_TOLERANCE_DEG)
...
def on_new_hdf5_file(
    ...
    try:
        ...
        logger.error(
            f"Error processing new HDF5 file {file_path}: {e}", exc_info=True)
