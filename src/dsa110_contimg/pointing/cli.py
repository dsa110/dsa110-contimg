...
def find_complete_groups(data_dir: Path, start_date: Optional[datetime] = None,
...
def get_declinations(data_dir: Path, timestamps: List[datetime],
                     jump_threshold_deg: float = 0.1) -> List[Dict]:
...
def plot_timeline(timestamps: List[datetime], output_path: Path,
                  data_dir: Optional[Path] = None,
                  jump_threshold_deg: float = 0.1) -> None:
...
def crossmatch_transits_pointings(name: str, input_dir: str, products_db: str,
                                  catalogs: List[str], *,
    ...
    return find_transit_groups(
        name, input_dir, products_db, catalogs,
...
def main():
    ...
    pt_parser.add_argument("--data-dir", type=Path, required=True,
                          help="Directory containing UVH5 files")
    pt_parser.add_argument("--output", type=Path, required=True,
                          help="Output PNG path")
    pt_parser.add_argument("--start-date", type=str,
                          help="Start date filter (YYYY-MM-DD)")
    pt_parser.add_argument("--end-date", type=str,
                          help="End date filter (YYYY-MM-DD)")
    pt_parser.add_argument("--jump-threshold-deg", type=float, default=0.1,
    ...
    cm_parser.add_argument("--name", required=True,
                          help="Calibrator name (e.g., '0834+555')")
    cm_parser.add_argument("--input-dir", default="/data/incoming",
                          help="Input directory with UVH5 files")
    cm_parser.add_argument("--products-db", default="/data/dsa110-contimg/state/products.sqlite3",
                          help="Products database path")
    cm_parser.add_argument("--catalog", action="append",
                          default=[
    ...
    cm_parser.add_argument("--dec-tolerance-deg", type=float, default=1.5,
                          help="Declination tolerance (degrees)")
    cm_parser.add_argument("--time-pad-minutes", type=float, default=10.0,
                          help="Time padding around transit (minutes)")
    cm_parser.add_argument("--days-back", type=int, default=21,
                          help="Days to search back")
    cm_parser.add_argument("--json", action="store_true",
                          help="Output as JSON")
    ...
    if args.cmd == "plot-timeline":
        ...
        plot_timeline(timestamps, args.output, args.data_dir,
        ...
        results = crossmatch_transits_pointings(
            args.name, args.input_dir, args.products_db, args.catalog,
