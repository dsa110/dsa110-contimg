...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--hdf5",
        required=True,
        help="Path to HDF5 file to read declination from"
    )
    ap.add_argument(
        "--dec-range",
        type=float,
    ...
    ap.add_argument(
        "--output",
        help="Output SQLite database path (auto-generated if not provided)"
    )
    ap.add_argument(
        "--min-flux-mjy",
        type=float,
        help="Minimum flux threshold in mJy (optional)"
    )
