...
from dsa110_contimg.database.data_config import (STAGE_BASE, USE_NEW_STRUCTURE,
...
def move_ms_to_calibrated(
    ...
    # Move the file
    if USE_NEW_STRUCTURE and ms_path.exists():
        new_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(ms_path), str(new_path))
        return new_path
    
    return ms_path  # Return original path if not using new structure
