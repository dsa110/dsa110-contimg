    ...
    try:
        with fits.open(fits_file_path) as hdul:
            ...
            if data.ndim > 2:
                data = data.squeeze()
                if data.ndim > 2:
                    LOG.warning(f"Image {fits_file_path} has >2 dimensions, taking first slice for profile.")
    ...
    try:
        with fits.open(fits_file_path) as hdul:
            ...
            for i in range(n_annuli):
                ...
                error_values.append(float(flux_std / np.sqrt(annulus_data.size)) if annulus_data.size > 0 else 0.0)
    ...
    try:
        ...
        if error_clean is not None:
            popt, pcov = optimize.curve_fit(
                ...
                sigma=error_clean,
                bounds=([0, distance_clean[0], 0, -np.inf], [np.inf, distance_clean[-1], np.inf, np.inf]),
            )
        else:
            popt, pcov = optimize.curve_fit(
                gaussian_model,
        ...
        n_params = 4
        reduced_chi_squared = chi_squared / (len(distance_clean) - n_params) if len(distance_clean) > n_params else np.nan
    ...
    try:
        ...
        if error_clean is not None:
            popt, pcov = optimize.curve_fit(
                ...
                distance_clean,
                flux_clean,
                p0=[amplitude_guess, center_guess, alpha_guess, beta_guess, background_guess],
                sigma=error_clean,
                bounds=([0, distance_clean[0], 0, 0.5, -np.inf], [np.inf, distance_clean[-1], np.inf, 10, np.inf]),
            )
        else:
            popt, pcov = optimize.curve_fit(
                moffat_model,
