...
def validate_wcs_4d(wcs: Optional[WCS], default_freq: float = 0.0, default_stokes: float = 0.0) -> Tuple[WCS, bool, Tuple[float, float]]:
...
def wcs_pixel_to_world_safe(wcs: WCS, x: float, y: float, is_4d: bool = None, defaults: Tuple[float, float] = (0.0, 0.0)) -> Tuple[float, float]:
...
def wcs_world_to_pixel_safe(wcs: WCS, ra: float, dec: float, is_4d: bool = None, defaults: Tuple[float, float] = (0.0, 0.0)) -> Tuple[float, float]:
...
def validate_image_shape(data: np.ndarray, min_size: int = 1, max_size: int = None) -> Tuple[int, int]:
