    ...
    with _connect(db_path) as conn:
        ...
        for r in rows:
            ...
            if has_calibrator is not None:
                ...
                has_cal = any(indicator in ms_path_lower for indicator in ['cal', 'calibrator', '3c', 'j1331'])
...
def get_image_fits(request: Request, image_id: int):
    ...
    if not fits_path or not Path(fits_path).exists():
        return HTMLResponse(status_code=404, content=f"FITS file not found for image {image_id}. Conversion may have failed.")
