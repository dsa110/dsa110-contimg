...
class CARTAServiceManager:
    ...
    def check_backend_health(self) -> bool:
        """Check if CARTA backend is healthy by verifying it serves content."""
        try:
            # CARTA doesn't have a traditional health endpoint, but serves its frontend on /
            response = requests.get(f"http://localhost:{CARTA_BACKEND_PORT}/", timeout=2)
            # Check if response contains CARTA frontend HTML
            return response.status_code == 200 and "CARTA" in response.text
        except Exception:
            return False
