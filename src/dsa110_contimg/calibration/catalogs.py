...
FIRST_CATALOG_BASE_URL = (
    "https://third.ucllnl.org/first/catalogs/"  # Example, verify actual URL
)
...
def resolve_vla_catalog_path(
    ...
    if prefer_sqlite:
        ...
        sqlite_candidates.append(
            Path.cwd() / "state" / "catalogs" / "vla_calibrators.sqlite3"
        )
...
def read_first_catalog(
    ...
    if first_catalog_path:
        if not os.path.exists(first_catalog_path):
            raise FileNotFoundError(
                f"FIRST catalog file not found: {first_catalog_path}"
            )
    ...
    if use_astroquery:
        try:
            ...
            if use_astroquery:
                print(
                    "Warning: astroquery not available. Install with: pip install astroquery"
                )
...
def read_rax_catalog(
    ...
    if use_astroquery:
        try:
            ...
            print(
                "Warning: RAX catalog is DSA-110 specific and not available via Vizier."
            )
            print("Falling back to cached file...")
        except ImportError:
            pass
    ...
    try:
        ...
        try:
            df = pd.read_sql_query(
                "SELECT name, ra_deg, dec_deg, flux_jy FROM vla_20cm", conn
            )
        except Exception:
            # Fallback to calibrators table if view doesn't exist
            df = pd.read_sql_query(
                "SELECT name, ra_deg, dec_deg FROM calibrators", conn
            )
            df["flux_jy"] = None
...
def calibrator_match(
    ...
    *,
    ...
    sep = np.hypot(
        (sel["ra_deg"] - ra_meridian) * cosd, (sel["dec_deg"] - dec_meridian)
    )
    sel["sep_deg"] = sep
...
def generate_caltable(
    ...
    ant_ra = (
        0.0  # use RA=self for beam centering approximation; drop explicit RA dependence
    )
    ...
    for name, row in cal_df.iterrows():
        ...
        field = vla_df[
            ...
            & (
                vla_df["ra"]
                < row["ra"]
                + radius.to_value(u.deg) / max(np.cos(np.deg2rad(pt_dec_deg)), 1e-3)
            )
            & (
                vla_df["ra"]
                > row["ra"]
    ...
    cal_df["percent_flux"] = cal_df["weighted_flux"] / cal_df["field_flux"].replace(
        0, np.nan
    )
    ...
    out = out.rename(
        columns={"code_20_cm": "code_20_cm", "ra": "ra(deg)", "dec": "dec(deg)"}
    )
    out = out[
        [
            "ra(deg)",
