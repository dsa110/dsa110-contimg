...
def cmd_inject(args: argparse.Namespace) -> int:
    ...
    print(f"Writing MODEL_DATA for {name} @ ra={ra_deg:.6f} dec={dec_deg:.6f} flux={flux_jy:.3f} Jy")
    try:
        write_point_model_with_ft(args.ms, ra_deg, dec_deg, flux_jy, reffreq_hz=reffreq_hz, spectral_index=sidx)
...
def main(argv=None) -> int:
    ...
    sp.add_argument('--reffreq', type=float, help='Reference frequency (Hz) for the component (default 1.4e9)')
