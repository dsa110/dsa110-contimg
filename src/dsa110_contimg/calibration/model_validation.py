...
def validate_model_data_populated(ms_path: str, min_fraction: float = 0.01, 
                                   sample_size: int = 1000) -> Tuple[bool, Dict]:
    ...
    try:
        with table(ms_path, readonly=True, ack=False) as tb:
            ...
            stats.update({
                ...
                "mean_value": float(np.mean(abs_values[nonzero_mask])) if np.any(nonzero_mask) else 0.0,
            ...
            with table(ms_path, readonly=True, ack=False) as tb:
                ...
                if len(unflagged_model) > 0:
                    ...
                    if catalog_flux_jy > 0:
                        ...
                        validation["flux_match"] = (
                            flux_ratio >= (1.0 - flux_tolerance_factor) and
                            flux_ratio <= (1.0 + flux_tolerance_factor)
                        )
...
def validate_model_data_consistency(ms_path: str, fields: Optional[List[int]] = None) -> Tuple[bool, Dict]:
...
def comprehensive_model_data_validation(
    ...
    results["all_valid"] = (
        results["exists"] and
        results["populated"] and
        (results["catalog_match"] is None or results["catalog_match"]) and
        results["consistent"]
    )
