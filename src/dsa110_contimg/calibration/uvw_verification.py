...
def get_uvw_statistics(ms_path: str, n_sample: int = 1000) -> dict:
    ...
    with table(ms_path, readonly=True) as main_tb:
        ...
        stats = {
            ...
            'baseline_length_range': (float(np.min(baseline_length)), float(np.max(baseline_length))),
        }
...
def verify_uvw_alignment(
    ...
    try:
        ...
        ms_coord = SkyCoord(ra=ms_phase_center[0]*u.deg, dec=ms_phase_center[1]*u.deg, frame='icrs')
        exp_coord = SkyCoord(ra=expected_phase_center[0]*u.deg, dec=expected_phase_center[1]*u.deg, frame='icrs')
