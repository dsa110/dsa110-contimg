...
DEFAULT_OUTRIGGER_PRIORITY = [
    104, 105, 106, 107, 108,  # Eastern (best coverage)
...
def analyze_antenna_health_from_caltable(
    caltable_path: str
) -> List[Dict[str, Any]]:
...
def recommend_outrigger_refants(
    ...
    outrigger_stats = [
        ...
        if ant['antenna_id'] in OUTRIGGER_ANTENNAS
    ]
    
    if not outrigger_stats:
        logger.warning(
            "No outrigger antennas found in antenna statistics"
        )
        recommendations['recommended_refant'] = DEFAULT_OUTRIGGER_PRIORITY[0]
        recommendations['recommended_refant_string'] = (
            recommendations['default_refant_string']
        ...
        recommendations['note'] = (
            "No outrigger stats found - using default priority"
        )
        return recommendations
...
def recommend_refants_from_ms(
    ...
    if not caltable_path:
        logger.info(
            "No calibration table provided - using default outrigger chain"
        )
