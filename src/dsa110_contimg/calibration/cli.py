...
def main():
    ...
    p = argparse.ArgumentParser(
        description="CASA 6.7 calibration runner")
...
def handle_find_calibrators_in_ms(args: argparse.Namespace, logger: logging.Logger) -> int:
        ...
        try:
            ...
            if dec_match.empty:
                ...
                if 'flux_20_cm' in dec_match.columns:
                    ...
                    dec_match = dec_match.sort_values(['weighted_flux', 'sep_deg'], ascending=[False, True])
                else:
    ...
    for cal_name, cal_info in all_calibrators.items():
        ...
        if ms_times:
            ...
            cal_info['delta_from_transit_minutes'] = min_delta_minutes if min_delta_minutes is not None else None
        ...
        for cal_name, cal_info in sorted(all_calibrators.items()):
            ...
            if cal_info.get('peak_transit_ms'):
                ...
                if cal_info.get('delta_from_transit_minutes') is not None:
                    logger.info(f"    Time from transit: {cal_info['delta_from_transit_minutes']:.1f} minutes")
            logger.info("")
