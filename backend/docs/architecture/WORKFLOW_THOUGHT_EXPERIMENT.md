...
**Location**: `streaming_converter.py` → `bootstrap_directory()` /
`_polling_loop()`
...
**Location**: `streaming_converter.py` → `_worker_loop()` →
...
5. MS file written to:
   `/stage/dsa110-contimg/ms/science/YYYY-MM-DD/<group_id>.ms/`
...
1. Register mosaic in `data_registry`:
   ...
   - `auto_publish=True`
2. Finalize data:
   - Call `finalize_data()` with `qa_status='passed'`,
     `validation_status='validated'`
   - This triggers `trigger_auto_publish()` if criteria met
3. Auto-publish:
   - Move mosaic from `/stage/dsa110-contimg/mosaics/` to
...
- `published_path` construction uses `stage_path_obj.name` which might lose
  directory structure
- Need to verify paths are resolved before database storage

### ICEBERG #16: Database Transaction Consistency

- `create_mosaic()` updates `mosaic_groups` but doesn't register in
  `data_registry`
...
- Mosaic validation happens in `validate_tiles_consistency()` but status not
  stored
...
- Need consistent naming between `mosaic_groups.mosaic_id` and
  `data_registry.data_id`
