...
def test_database_queries():
    ...
    # Navigate from tests/unit/api/ to src/dsa110_contimg/api/
    routes_path = Path(__file__).parents[2] / ".." / "src" / "dsa110_contimg" / "api" / "routes.py"
    routes_path = routes_path.resolve()
    if not routes_path.exists():
        pytest.skip(f"routes.py not found at {routes_path}")
    content = routes_path.read_text()
