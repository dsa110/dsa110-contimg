...
class TestCalculateSigmaDeviation:
    ...
    def test_precomputed_statistics(self):
        ...
        result_with_stats = calculate_sigma_deviation(fluxes, mean=actual_mean, std=actual_std)
        result_auto = calculate_sigma_deviation(fluxes)

        # Assert: Should give same result
        assert (
            abs(result_with_stats - result_auto) < 0.01
    ...
    def test_sigma_deviation_smoke(self):
        ...
        assert abs(result - expected_max_dev) < 0.1, f"Expected ≈ {expected_max_dev}, got {result}"
