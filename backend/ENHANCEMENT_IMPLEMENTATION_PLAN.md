...
This document outlines the implementation plan for the future enhancements identified in `ARCHITECTURE_REFACTORING.md`. The enhancements will improve the backend's robustness, maintainability, and performance.

## Current State Analysis

### ✅ Already Implemented
1. **Async Database Operations** - Partially complete
...
1. **Async Database Operations**
   - Routes still use sync repositories in many places
   - Need to migrate route handlers to use async repositories
   - Service layer needs async conversion

2. **Connection Pooling**
   - Basic pooling exists in `database.py`
   - Could be enhanced with better lifecycle management
   - Need connection pool monitoring/metrics

3. **FITS Parsing Service**
   - FITS parsing logic scattered in repositories
   - Should be extracted to dedicated service
   - `services/fits_service.py` exists but not fully integrated

4. **Narrow Exception Handling**
   - 86 instances of `except Exception:` found
   - Many are legitimate (fallback scenarios, optional features)
   - Some should be narrowed to specific exception types
