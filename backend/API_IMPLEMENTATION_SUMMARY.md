...
- ✓ Health check
- ✓ MS metadata and calibrator matches
- ✓ Source detail
- ✓ Job provenance and logs
- ✓ QA endpoints (image, MS, job)
- ✓ Calibration table detail
- ✓ Error responses (404s) with proper envelopes
...
```
ms_index.path → MSDetailResponse.path
ms_index.pointing_ra_deg → MSDetailResponse.pointing_ra_deg
ms_index.pointing_dec_deg → MSDetailResponse.pointing_dec_deg
ms_index.stage → MSDetailResponse.qa_grade (transformed)
caltables.* → MSDetailResponse.calibrator_matches
```

**Image Metadata:**

```
images.id → ImageDetailResponse.id
images.path → ImageDetailResponse.path
images.center_ra_deg → ImageDetailResponse.pointing_ra_deg
images.center_dec_deg → ImageDetailResponse.pointing_dec_deg
images.noise_jy → QA summary calculation
images.dynamic_range → QA summary calculation
```

**Source Data:**

```
photometry.source_id → SourceDetailResponse.id
photometry.ra_deg → SourceDetailResponse.ra_deg
photometry.dec_deg → SourceDetailResponse.dec_deg
photometry.image_path → ContributingImage.path
```
...
- `imaged`, `mosaicked`, `cataloged` → `"good"`
- `calibrated` → `"warn"`
- `discovered`, `null`, or other → `"fail"`
