...
### Semi-Complete Subband Groups (NEW)

**Important**: The pipeline now supports **semi-complete groups** (12-15
subbands). Missing subbands are automatically replaced with zero-padded
synthetic subbands that are fully flagged.

**Behavior**:

- Groups with **12-16 subbands** are accepted (was: only 16/16)
- Missing subbands (1-4) are automatically filled with synthetic zero-padded
  files
- Synthetic subbands are fully flagged (no bad data in images)
- Metadata tracks group completeness (`SubbandGroupInfo`)

**See**: `conversion/SEMI_COMPLETE_SUBBAND_GROUPS.md` for complete documentation
...
- `conversion/SEMI_COMPLETE_SUBBAND_GROUPS.md`: Semi-complete group protocol
  documentation
- `database/hdf5_index.py`: `SubbandGroupInfo` dataclass and group querying
