    ...
    *,
    ...
    try:
        ...
        with table(f"{ms_in}::FIELD", nomodify=True) as field_in:
            ...
            if nfields > 1:
                print(
                    f":check_mark: Validated: all {nfields} fields share the same phase center (max separation: {max_separation_arcsec:.3f} arcsec)"
                )
        ...
        print(f":check_mark: Successfully merged {nfields} fields into 1 field")
...
if __name__ == "__main__":
    ...
    if n_spw_out:
        ...
        if n_spw_out == 1:
            print(":check_mark: Successfully merged SPWs into single SPW")
        else:
            print(f":warning_sign: Warning: Expected 1 SPW, got {n_spw_out}")
