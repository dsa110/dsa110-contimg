...
def create_catalog_mask(
    ...
    catalog: Union[CatalogName, str] = CatalogName.UNICAT,
    ...
    Args:
        image_path: Path to input FITS image (for WCS/FoV)
        catalog: Catalog to use (default: unicat). Options: nvss, first, vlass, unicat, atnf, rax
        ...
        out_path: Output path (auto-generated if not specified)
        output_format: "crtf" for CASA region file, "fits" for FITS mask
...
def create_catalog_fits_mask(
    ...
    Args:
        ...
        catalog: Catalog to use (default: unicat)
        min_flux_mjy: Minimum flux in mJy (uses catalog default if not specified)
        radius_arcsec: Mask radius around each source in arcseconds
        out_path: Optional output path (defaults to {imagename}.{catalog}_mask.fits)
...
def create_catalog_overlay(
    ...
    pb_path: Optional[Union[str, Path]] = None,
    pblimit: float = 0.2,
) -> None:
    """Create a diagnostic overlay image with catalog sources marked.

    Args:
        image_path: Input FITS image path
        out_path: Output PNG path
        catalog: Catalog to overlay (default: unicat)
        min_flux_mjy: Minimum flux in mJy to plot
        pb_path: Optional primary beam FITS for masking detections
        pblimit: PB cutoff when pb_path is provided
