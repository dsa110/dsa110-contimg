...
def _submit_imaging_tasks(executor, ms_path: str, imgroot: Path, out_dir: Path, use_gpu: bool):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            future_deep, future_fast, future_gpu = _submit_imaging_tasks(
                executor, ms_path, imgroot, out_dir, use_gpu
            )
...
def _get_ms_time_info(ms: Path) -> Tuple[Optional[float], Optional[float], float]:
...
def _process_single_ms(ms: Path, out_dir: Path, registry_db: Path, conn) -> bool:
...
def process_once(ms_dir: Path, out_dir: Path, registry_db: Path, products_db: Path) -> int:
    ...
    for ms in sorted(ms_dir.glob("**/*.ms")):
        ...
        if _process_single_ms(ms, out_dir, registry_db, conn):
            processed += 1
