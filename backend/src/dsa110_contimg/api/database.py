...
class DatabasePool:
    ...
    async def _get_connection(self) -> aiosqlite.Connection:
        ...
        conn = await aiosqlite.connect(self.config.db_path, timeout=self.config.timeout)
...
@asynccontextmanager
async def async_transaction(conn: aiosqlite.Connection) -> AsyncIterator[aiosqlite.Connection]:
    """
...
def transactional_connection(
    db_path: str, timeout: float = 30.0, row_factory: bool = True
...
async def async_transactional_connection(
    db_path: str, timeout: float = 30.0
