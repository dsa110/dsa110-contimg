...
class AsyncImageService:
    ...
    def build_provenance_links(self, image: "ImageRecord") -> dict:
        ...
        return {
            ...
            "ms_url": (
                f"/api/ms/{quote(image.ms_path, safe='')}/metadata" if image.ms_path else None
            ),
...
class AsyncMSService:
    ...
    async def list_ms(self, limit: int = 100, offset: int = 0) -> List["MSRecord"]:
...
class AsyncSourceService:
    ...
    async def list_sources(self, limit: int = 100, offset: int = 0) -> List["SourceRecord"]:
    ...
    def calculate_variability(self, source: "SourceRecord", epochs: List[dict]) -> dict:
        """
        ...
        if errors and len(errors) == len(fluxes):
            chi_squared = sum(((f - mean_flux) / e) ** 2 for f, e in zip(fluxes, errors) if e > 0)
            dof = len(fluxes) - 1
