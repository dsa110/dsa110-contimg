...
async def get_ms_raster(
    encoded_path: str,
    xaxis: Literal["time", "baseline", "frequency"] = Query("time", description="X-axis dimension"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate raster plot: {str(e)}")
...
def _get_antenna_info(ms_path: str) -> AntennaLayoutResponse:
    ...
    center_lat = np.degrees(
        np.arctan2(center_xyz[2], np.sqrt(center_xyz[0] ** 2 + center_xyz[1] ** 2))
    )
...
async def get_antenna_layout(encoded_path: str) -> AntennaLayoutResponse:
    ...
    if not Path(f"{ms_path}/ANTENNA").exists():
        raise HTTPException(status_code=404, detail=f"ANTENNA subtable not found in {ms_path}")

    try:
        return _get_antenna_info(ms_path)
    except Exception as e:
        logger.exception(f"Error reading antenna info from {ms_path}")
        raise HTTPException(status_code=500, detail=f"Failed to read antenna information: {str(e)}")
