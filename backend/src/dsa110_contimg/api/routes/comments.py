...
async def get_comment_stats(
    ...
    today_count = db.execute(
        "SELECT COUNT(*) FROM user_comments WHERE created_at >= ?",
        (today,)
    ).fetchone()[0]
...
async def get_mentionable_users(
    ...
    return [
        MentionableUser(user_id=row[0], username=row[1])
        for row in cursor.fetchall()
    ]
...
async def list_comments(
    ...
    parent_id: Optional[str] = Query(None, description="Filter by parent (use 'null' for top-level)"),
...
async def get_comment(
    ...
    reply_count = db.execute(
        "SELECT COUNT(*) FROM user_comments WHERE parent_id = ?",
        (comment_id,)
    ).fetchone()[0]
...
async def create_comment(
    ...
    db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (comment_id, data.target_type, data.target_id, user_id, username, data.content, data.parent_id, now, now),
    )
    db.commit()
