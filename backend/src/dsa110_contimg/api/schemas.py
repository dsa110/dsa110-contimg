...
class ImageDetailResponse(BaseModel):
    ...
    qa_grade: Optional[Literal["good", "warn", "fail"]] = Field(
        None, description="QA assessment grade"
    )
    ...
    parent_id: Optional[str] = Field(
        None, description="ID of parent image (for re-imaged products)"
    )
...
class QAReportResponse(BaseModel):
    ...
    qa_grade: Optional[Literal["good", "warn", "fail"]] = Field(
        None, description="Overall QA grade"
    )
    qa_summary: Optional[str] = Field(None, description="Brief QA summary")
...
class RasterPlotParams(BaseModel):
    ...
    xaxis: Literal["time", "baseline", "frequency"] = Field("time", description="X-axis dimension")
...
class AntennaInfo(BaseModel):
    ...
    flagged_pct: float = Field(
        0.0, ge=0.0, le=100.0, description="Percentage of data flagged for this antenna"
    )
