...
def get_job_service(repo: AsyncJobRepository = Depends(get_job_repository)) -> "AsyncJobService":
...
def get_ms_service(repo: AsyncMSRepository = Depends(get_ms_repository)) -> "AsyncMSService":
...
def get_stats_service(db_pool: DatabasePool = Depends(get_db_pool)) -> StatsService:
