...
class RFIDetectionConfig:
    ...
    Attributes:
        threshold: Detection threshold in MAD units (default 5.0)
        gpu_id: GPU device ID to use (default 0)
        chunk_size: Number of visibilities per processing chunk (default 10M)
        apply_flags: Whether to apply flags to the MS (default True)
        detect_only: Only detect, don't modify MS (default False)
...
class RFIDetectionResult:
    ...
    Attributes:
        ...
        total_vis: Total number of visibilities
        flagged_vis: Number of newly flagged visibilities
        flag_percent: Percentage of data flagged
        threshold: Detection threshold used (MAD units)
...
def _detect_outliers_cupy(
    ...
    Args:
        vis_data: Complex visibility data (any shape, will be flattened)
...
def gpu_rfi_detection(
    ms_path: str,
    *,
    config: Optional[RFIDetectionConfig] = None,
    ...
    Args:
        ...
        threshold: Detection threshold in MAD units (overrides config)
        gpu_id: GPU device ID (overrides config)
        chunk_size: Visibilities per chunk (overrides config)

    Returns:
        RFIDetectionResult with detection statistics
