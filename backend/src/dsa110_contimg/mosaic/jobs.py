"""
ABSURD job implementations for mosaicking pipeline.
...
class MosaicPlanningJob(Job):
    ...
    def execute(self) -> JobResult:
        ...
        try:
            ...
            ra_values = [img['center_ra_deg'] for img in images 
                        if img['center_ra_deg'] is not None]
            dec_values = [img['center_dec_deg'] for img in images
                         if img['center_dec_deg'] is not None]
...
class MosaicBuildJob(Job):
    ...
    def execute(self) -> JobResult:
        ...
        try:
            ...
            cursor = conn.execute(
                f"SELECT path FROM {self.config.images_table} "
            ...
            cursor = conn.execute(
                ...
                (self.plan_id, str(output_path), plan['tier'],
                 len(image_paths), result.median_rms, 
                 result.coverage_sq_deg, int(time.time()))
            )
        ...
        try:
            ...
            conn.execute(
                ...
                (self.mosaic_id, qa_result.astrometry_rms, qa_result.n_stars,
                 ...
                 int(qa_result.has_artifacts), qa_result.artifact_score,
                 int(qa_result.passed), json.dumps(qa_result.warnings),
                 int(time.time()))
            )
