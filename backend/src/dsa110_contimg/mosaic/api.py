...
def _get_config() -> dict[str, Any]:
    ...
    if not _config:
        raise HTTPException(
            status_code=500,
            detail="Mosaic API not configured. Call configure_mosaic_api() first."
        )
    return _config
...
async def create_mosaic(request: MosaicRequest) -> MosaicResponse:
    ...
    if request.end_time <= request.start_time:
        raise HTTPException(
            status_code=400,
            detail="Invalid time range: end_time must be after start_time"
        )
    ...
    if request.tier.lower() not in valid_tiers:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid tier: {request.tier}. Must be one of: {valid_tiers}"
        )
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT id FROM mosaic_plans WHERE name = ?",
            (request.name,)
        )
        ...
        if existing:
            ...
            raise HTTPException(
                status_code=409,
                detail=f"Mosaic with name '{request.name}' already exists"
            )
    ...
    try:
        from .pipeline import MosaicPipelineConfig, execute_mosaic_pipeline_task
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to start mosaic creation: {e}"
        )
...
async def get_mosaic_status(name: str) -> MosaicStatusResponse:
    ...
    try:
        ...
        if not plan:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Mosaic '{name}' not found"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get mosaic status: {e}"
        )
...
async def list_mosaics(
    ...
    try:
        ...
        for plan in plans:
            ...
            if plan["status"] == "completed":
                cursor = conn.execute(
                    "SELECT path, qa_status FROM mosaics WHERE plan_id = ?",
                    (plan["id"],)
                )
                mosaic = cursor.fetchone()
                if mosaic:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list mosaics: {e}"
        )
...
async def delete_mosaic(name: str, delete_file: bool = False) -> dict[str, str]:
    ...
    try:
        ...
        if mosaic:
            conn.execute(
                "DELETE FROM mosaic_qa WHERE mosaic_id = ?",
                (mosaic["id"],)
            )
            conn.execute(
                "DELETE FROM mosaics WHERE id = ?",
                (mosaic["id"],)
            )
        
        conn.execute(
            "DELETE FROM mosaic_plans WHERE id = ?",
            (plan_id,)
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to delete mosaic: {e}"
        )
