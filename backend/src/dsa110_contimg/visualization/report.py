...
def generate_html_report(
    ...
    ver_line = f"<p>Pipeline Version: {metadata.pipeline_version}</p>" if metadata.pipeline_version else ""
...
def generate_pdf_report(
    ...
    with PdfPages(output) as pdf:
        ...
        fig.text(0.5, 0.55, f"Generated: {metadata.date.strftime('%Y-%m-%d %H:%M:%S')}", 
                 ha="center", va="center", fontsize=12)
        fig.text(0.5, 0.5, f"Author: {metadata.author}", ha="center", va="center", fontsize=12)
        if metadata.observation_id:
            fig.text(0.5, 0.45, f"Observation: {metadata.observation_id}", ha="center", va="center", fontsize=12)
        fig.text(0.5, 0.35, f"Pipeline Version: {metadata.pipeline_version}", ha="center", va="center", fontsize=10, color="gray")
        pdf.savefig(fig)
        plt.close(fig)
        ...
        for section in sections:
            ...
            if section.content:
                fig = plt.figure(figsize=(8.5, 11))
                fig.text(0.5, 0.9, section.title, ha="center", va="top", fontsize=18, fontweight="bold")
            ...
            for caltable in caltable_dir.glob(pattern):
                try:
                    ...
                    from dsa110_contimg.visualization.calibration_plots import (
                        plot_bandpass, plot_gains, plot_delays
                    )
