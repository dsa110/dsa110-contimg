...
def plot_tile_grid(
    ...
    Args:
        tiles: List of tile FITS file paths
        output: Output file path
        config: Figure configuration
        ncols: Number of columns in grid
        show_labels: Show tile names
        stretch: Image stretch ('linear', 'log', 'sqrt', 'asinh')
    ...
    from astropy.visualization import ImageNormalize, AsinhStretch, LogStretch, SqrtStretch
    ...
    for idx, tile_path in enumerate(tiles):
        ...
        try:
            ...
            ax.imshow(
                data, origin="lower", cmap=config.colormap,
                norm=norm, vmin=vmin if not stretch_fn else None,
...
def plot_mosaic_footprints(
    ...
    Args:
        ...
        output: Output file path
        config: Figure configuration
        mosaic_wcs: WCS of the final mosaic (for coordinate frame)
        mosaic_shape: Shape of the final mosaic
        title: Plot title
    ...
    for idx, tile_path in enumerate(tiles):
        ...
        try:
            ...
            if mosaic_wcs is not None:
                ...
                ax.text(cx[0], cy[0], tile_path.stem, fontsize=6, ha="center", va="center", color=colors[idx])
            else:
                ax.text(center_ra, center_dec, tile_path.stem, fontsize=6, ha="center", va="center", color=colors[idx])
...
def plot_coverage_map(
    ...
    Args:
        ...
        wcs: WCS for coordinate display
        output: Output file path
        config: Figure configuration
        title: Plot title
