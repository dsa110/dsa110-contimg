...
def _get_2d_data_and_wcs(fits_path: Union[str, Path]) -> tuple["NDArray", "WCS", dict]:
    ...
    Returns:
        Tuple of (data_2d, wcs_2d, header_dict)
    """
...
def _normalize_image(
    ...
    Args:
        data: 2D image array
        vmin: Manual minimum value
        vmax: Manual maximum value
        stretch: Stretch function ('linear', 'asinh', 'log', 'sqrt')
        contrast: ZScale contrast parameter
...
def plot_fits_image(
    ...
    # Normalize
    norm, vmin_used, vmax_used = _normalize_image(
        data, vmin=vmin, vmax=vmax, stretch=stretch
    )
...
def plot_cutout(
    ...
    **kwargs,
    ...
    Args:
        fits_path: Path to FITS file
        ra: Right ascension in degrees (or use coord)
        dec: Declination in degrees (or use coord)
        coord: SkyCoord object (alternative to ra/dec)
        radius_arcmin: Cutout radius in arcminutes
        output: Output file path
        config: Figure configuration
        title: Plot title
        show_crosshair: Show crosshair at center
        show_circle: Show circle around source
        circle_radius_arcsec: Radius of circle in arcseconds
        **kwargs: Additional arguments passed to plot_fits_image
    ...
    # Normalize
    norm, _, _ = _normalize_image(data, **{k: v for k, v in kwargs.items() 
                                           if k in ['vmin', 'vmax', 'stretch']})
    ...
    if show_crosshair:
        ax.scatter(
            ...
            marker="+", c="red", s=100, linewidths=2,
...
def save_quicklook_png(
    ...
    Args:
        fits_path: Path to FITS file
        output: Output PNG path (default: fits_path + '.png')
        max_size: Maximum dimension before downsampling
...
def plot_mosaic_overview(
    ...
    Args:
        mosaic_path: Path to mosaic FITS
        weight_path: Path to weight map FITS
        output: Output file path
        config: Figure configuration
    ...
    norm, _, _ = _normalize_image(data)
    im1 = ax1.imshow(
        data, origin="lower", cmap=config.cmap, norm=norm, interpolation="nearest"
    )
    ...
    if has_weights:
        im2 = ax2.imshow(
            weight_data, origin="lower", cmap="viridis", interpolation="nearest"
        )
