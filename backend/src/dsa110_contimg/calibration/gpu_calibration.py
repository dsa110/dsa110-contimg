...
class CalibrationConfig:
    ...
    Attributes:
        ...
        chunk_size: Number of visibilities per processing chunk (default 5M)
...
class GainSolutionResult:
    ...
    Attributes:
        gains: Complex gain array (n_antennas, n_channels, n_pols)
        weights: Solution weights/confidence (n_antennas, n_channels, n_pols)
        n_iterations: Number of iterations performed
        converged: Whether solution converged
        residual_rms: Final residual RMS
...
class ApplyCalResult:
    ...
    Attributes:
        n_vis_processed: Total visibilities processed
        n_vis_calibrated: Number of visibilities successfully calibrated
        n_vis_flagged: Number of visibilities flagged (bad gains)
        processing_time_s: Processing time in seconds
        gpu_id: GPU device used
...
def _get_kernel(name: str) -> Any:
    ...
    Args:
        name: Kernel name ('apply_gains' or 'solve_gains')
...
def apply_gains_gpu(
    ...
    *,
    ...
    Args:
        vis: Complex visibilities (n_vis, n_pol) or (n_vis,)
        gains: Complex gains (n_ant, n_chan, n_pol) or (n_ant,)
        ant1: Antenna 1 indices (n_vis,)
        ant2: Antenna 2 indices (n_vis,)
...
def solve_per_antenna_gains_cpu(
    ...
    *,
    ...
    Args:
        vis: Observed visibilities (n_vis,) complex
        model: Model visibilities (n_vis,) complex
        ant1: Antenna 1 indices
        ant2: Antenna 2 indices
        ...
        refant: Reference antenna index (gain fixed to 1+0j)
