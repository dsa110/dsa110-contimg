...
def compute_calibration_metrics(
    ...
    Args:
        ...
        cal_type: Type of calibration ('k', 'bp', 'g'). Auto-detected if None.

    Returns:
        CalibrationMetrics dataclass with extracted metrics
    """
...
def assess_calibration_quality(
    ...
    *,
    caltables: Optional[Dict[str, Optional[str]]] = None,
) -> CalibrationQAResult:
    ...
    Args:
        ...
        thresholds: QA thresholds (uses defaults if None)
        caltables: Pre-discovered caltables dict. If None, discovers them.

    Returns:
        CalibrationQAResult with pass/fail status and detailed metrics
    """
    start_time = time.time()
...
class CalibrationQAStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to SQLite database. Uses pipeline.sqlite3 if None.
        """
        if db_path is None:
    ...
    def _init_db(self) -> None:
        ...
        with sqlite3.connect(self.db_path, timeout=30.0) as conn:
            ...
            conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_calibration_qa_ms_path
                ON calibration_qa(ms_path)
                """
            )
            conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_calibration_qa_created_at
                ON calibration_qa(created_at DESC)
                """
            )
            conn.commit()
    ...
    def save_result(self, result: CalibrationQAResult) -> int:
        ...
        Args:
            result: CalibrationQAResult to save
    ...
    def get_result(self, ms_path: str) -> Optional[CalibrationQAResult]:
        ...
        Returns:
            Most recent CalibrationQAResult or None
        """
        with sqlite3.connect(self.db_path, timeout=30.0) as conn:
            conn.row_factory = sqlite3.Row
            row = conn.execute(
                """
                SELECT result_json FROM calibration_qa
                WHERE ms_path = ?
    ...
    def list_recent(
        ...
        *,
        ...
        Args:
            ...
            min_grade: Minimum grade to include ('excellent', 'good', 'marginal', 'poor')

        Returns:
            List of CalibrationQAResult
        """
        conditions = []
        params: List[Any] = []
    ...
    def get_summary_stats(self, since_timestamp: Optional[float] = None) -> Dict[str, Any]:
        """Get summary statistics for QA results.

        Args:
            since_timestamp: Only include results after this timestamp
    ...
    def cleanup_old_results(self, days: int = 30) -> int:
        ...
        with sqlite3.connect(self.db_path, timeout=30.0) as conn:
            cursor = conn.execute(
                "DELETE FROM calibration_qa WHERE created_at < ?",
                (cutoff,),
            )
            conn.commit()
...
_qa_store: Optional[CalibrationQAStore] = None


def get_qa_store(db_path: Optional[str] = None) -> CalibrationQAStore:
