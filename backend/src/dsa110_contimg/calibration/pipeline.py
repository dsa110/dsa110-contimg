...
class CalibrationPipelineConfig:
    ...
    Attributes:
        ...
        caltable_dir: Directory for output calibration tables
        catalog_path: Path to calibrator catalog (optional)
        default_refant: Default reference antenna chain
        do_k_calibration: Whether to perform K (delay) calibration
    """
...
class CalibrationResult:
    ...
    Attributes:
        ...
        solve_id: ID of the calibration solve record
        application_id: ID of the calibration application record
        validation_passed: Whether QA validation passed
        gaintables: List of produced calibration table paths
...
class CalibrationPipeline(Pipeline):
    ...
    def __init__(
        ...
        Args:
            ...
            cal_field: Calibrator field name (auto-detected if None)
            target_field: Target field to apply calibration to
...
class StreamingCalibrationPipeline(Pipeline):
    ...
    Example:
        ...
        >>>
        >>> def on_data_ingested(event):
        ...     if event.data.get("has_calibrator"):
        ...         pipeline = StreamingCalibrationPipeline(
        ...             config=config,
        ...             ms_path=event.data["ms_path"],
        ...         )
        ...         await executor.execute(pipeline)
        >>>
        >>> emitter = EventEmitter.get_instance()
        >>> emitter.subscribe(EventType.DATA_INGESTED, on_data_ingested)
    """
    ...
    def __init__(
        ...
        Args:
            config: Pipeline configuration
            ms_path: Path to Measurement Set with calibrator data
            refant: Reference antenna (auto-detected if None)
        """
        self.ms_path = ms_path
        self.refant = refant
...
async def run_calibration_pipeline(
    ...
    Convenience function for running calibration without manually
    setting up executor.

    Args:
        ms_path: Path to Measurement Set
        target_field: Target field to calibrate
        config: Pipeline configuration
