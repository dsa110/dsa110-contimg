"""
ABSURD job implementations for calibration pipeline.
...
class CalibrationJobConfig:
    ...
    Attributes:
        ...
        catalog_path: Path to calibrator catalog (optional, auto-resolved)
    """
...
class CalibrationSolveJob(Job):
    ...
    def execute(self) -> JobResult:
        ...
        # Emit calibration started event
        emitter = EventEmitter.get_instance()
        emitter.emit(
            EventType.CALIBRATION_STARTED,
            {"ms_path": self.ms_path, "do_k": self.do_k},
            source="calibration_solve_job",
        )
        ...
        try:
            ...
            if not success:
                ...
                emitter.emit(
                    ...
                    {"ms_path": self.ms_path, "error": error_msg},
                    source="calibration_solve_job",
                )
            ...
            emitter.emit(
                ...
                {"ms_path": self.ms_path, "solve_id": solve_id, **outputs},
                source="calibration_solve_job",
            )
            ...
            emitter.emit(
                EventType.CALIBRATION_FAILED,
                {"ms_path": self.ms_path, "error": str(e)},
                source="calibration_solve_job",
            )
...
class CalibrationValidateJob(Job):
    ...
    def execute(self) -> JobResult:
        ...
        if not validation_passed:
            emitter.emit(
                EventType.MOSAIC_QA_FAILED,  # Reuse QA event type
                {
                    "ms_path": self.ms_path,
                    "validation_type": "calibration",
                    "warnings": warnings,
                },
                source="calibration_validate_job",
            )
