...
Example:
    >>> from dsa110_contimg.calibration.catalog_registry import query_catalog, CatalogName
...
class CatalogConfig:
    ...
    Attributes:
        ...
        db_pattern: Filename pattern for declination-specific databases
        full_db_name: Filename for full-sky database (fallback)
        flux_column: Column name for flux in mJy
        ra_column: Column name for RA in degrees
        dec_column: Column name for Dec in degrees
        default_min_flux_mjy: Default minimum flux threshold
        extra_columns: Additional columns to include in query results
    """
...
CATALOG_REGISTRY: dict[CatalogName, CatalogConfig] = {
    ...
    CatalogName.UNICAT: CatalogConfig(
        ...
        description="Unified catalog (crossmatched NVSS+FIRST+VLASS)",
        db_pattern="master_sources.sqlite3",  # Single file, not dec-specific
        full_db_name="master_sources.sqlite3",
        flux_column="s_nvss",  # Flux in Jy, will convert to mJy
        default_min_flux_mjy=5.0,
        extra_columns=["snr_nvss", "s_vlass", "alpha", "resolved_flag", "confusion_flag"],
    ),
}
...
def _resolve_catalog_path(
    ...
    Args:
        catalog: Which catalog to find
        dec_deg: Declination in degrees (for dec-specific databases)
...
def query_catalog(
    ...
    Args:
        catalog: Catalog to query (CatalogName enum or string like "nvss", "unicat")
        ra_deg: Field center RA in degrees
        dec_deg: Field center Dec in degrees
        radius_deg: Search radius in degrees
        min_flux_mjy: Minimum flux in mJy (uses catalog default if not specified)
        max_sources: Maximum number of sources to return (sorted by flux, descending)
        include_extra_columns: If True, include catalog-specific extra columns

    Returns:
        DataFrame with standardized columns: ra_deg, dec_deg, flux_mjy, catalog
        Plus any extra_columns if include_extra_columns=True

    Raises:
        ValueError: If catalog is unknown
        FileNotFoundError: If catalog database cannot be found
...
def query_multiple_catalogs(
    ...
    radius_deg: float,
    min_flux_mjy: Optional[float] = None,
    max_sources_per_catalog: Optional[int] = None,
    deduplicate_radius_arcsec: float = 10.0,
) -> pd.DataFrame:
    """Query multiple catalogs and optionally deduplicate.

    Args:
        catalogs: List of catalogs to query
        ra_deg: Field center RA in degrees
        dec_deg: Field center Dec in degrees
        radius_deg: Search radius in degrees
        min_flux_mjy: Minimum flux in mJy
        max_sources_per_catalog: Max sources per catalog before merge
        deduplicate_radius_arcsec: Matching radius for deduplication (0 to disable)
...
def list_available_catalogs() -> list[dict]:
    ...
    Returns:
        List of dicts with catalog info: name, description, available, db_path
    """
    result = []
