...
def find_nearest_calibration(
    ...
    *,
    ...
    require_types: Optional[List[str]] = None,
    ...
    Args:
        ...
        calibration_dir: Root directory containing calibration tables (filesystem search)
        search_window_hours: Maximum time difference (default: ±24h)
        registry_db: Path to calibration registry database (default: pipeline.sqlite3)
        require_types: Required table types (default: ['bp', 'g'])
...
def _search_registry(
    ...
    try:
        ...
        for row in rows:
            ...
            ttype = row["table_type"].lower()
            if "bandpass" in ttype or ttype == "bp":
                sets[set_name]["tables"]["bp"] = row["path"]
            elif "delay" in ttype or ttype == "k":
                sets[set_name]["tables"]["k"] = row["path"]
            elif "gain" in ttype or ttype == "g":
                sets[set_name]["tables"]["g"] = row["path"]
...
def get_applylist_for_mjd(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to find_nearest_calibration

    Returns:
        List of calibration table paths to apply (ordered: k, bp, g)
    """
...
def check_calibration_staleness(
    ...
    *,
    ...
    Args:
        target_mjd: MJD to check calibration health for
        registry_db: Path to calibration registry
        warning_threshold_hours: Hours until warning (default: 12)
        critical_threshold_hours: Hours until critical (default: 24)
