...
class FluxScaleResult:
    ...
    Attributes:
        n_matched: Number of sources matched between image and catalog
        mean_flux_ratio: Mean ratio of measured flux to catalog flux
        rms_flux_ratio: RMS scatter in flux ratio
        flux_scale_error: Fractional flux scale error (|1 - mean_ratio|)
        has_issues: True if validation found significant issues
        issues: List of issue descriptions
        has_warnings: True if validation found non-critical warnings
        warnings: List of warning descriptions
        median_flux_ratio: Median flux ratio (more robust than mean)
        matched_sources: DataFrame of matched sources with flux comparisons
    """
    ...
    matched_sources: Optional[pd.DataFrame] = None
...
class AstrometryResult:
    ...
    Attributes:
        n_matched: Number of sources matched
        rms_offset_arcsec: RMS positional offset in arcseconds
        median_offset_arcsec: Median positional offset
        ra_offset_arcsec: Mean RA offset (positive = image shifted East)
        dec_offset_arcsec: Mean Dec offset (positive = image shifted North)
        has_issues: True if astrometry is significantly off
        issues: List of issue descriptions
        has_warnings: True if minor astrometric issues detected
        warnings: List of warning descriptions
    """
...
class SourceCountsResult:
    ...
    Attributes:
        n_detected: Number of sources detected in image
        n_expected: Number of sources expected from catalog
        n_matched: Number of matched sources
        completeness: Fraction of expected sources that were detected
        false_positive_rate: Fraction of detected sources not in catalog
        has_issues: True if completeness or false positive rate is problematic
        issues: List of issue descriptions
        has_warnings: True if minor issues detected
        warnings: List of warning descriptions
    """
...
def extract_sources_from_image(
    ...
    Args:
        ...
        rms_box_size: Box size for local RMS estimation (pixels)

    Returns:
        DataFrame with columns: ra_deg, dec_deg, flux_jy, snr, x_pix, y_pix
    ...
    # Handle multi-dimensional data (e.g., [1,1,ny,nx] from CASA)
    while data.ndim > 2:
        data = data[0]
...
def validate_flux_scale(
    ...
    detected_sources: Optional[pd.DataFrame] = None,
    catalog_sources: Optional[pd.DataFrame] = None,
) -> FluxScaleResult:
    ...
    Args:
        image_path: Path to FITS image (used for WCS and source extraction)
        catalog: Reference catalog name ("nvss", "first", "vlass", "racs")
        min_snr: Minimum SNR for source detection
        flux_range_jy: Flux range (min, max) in Jy to consider for matching
        max_flux_ratio_error: Maximum acceptable flux ratio error (0.2 = 20%)
        match_radius_arcsec: Cross-match radius in arcseconds
        detected_sources: Pre-extracted sources (optional, extracted if None)
        catalog_sources: Pre-queried catalog sources (optional, queried if None)

    Returns:
        FluxScaleResult with flux scale validation metrics
...
def _validate_astrometry(
    ...
    Args:
        detected_sources: DataFrame with ra_deg, dec_deg columns
        catalog_sources: Reference catalog DataFrame
        match_radius_arcsec: Cross-match radius in arcseconds
        max_rms_arcsec: Maximum acceptable RMS offset

    Returns:
        AstrometryResult with positional offset metrics
    """
...
def _validate_source_counts(
    ...
    Args:
        detected_sources: DataFrame with detected sources
        catalog_sources: Reference catalog DataFrame
        min_flux_jy: Minimum flux for completeness calculation
        match_radius_arcsec: Cross-match radius in arcseconds
        min_completeness: Minimum acceptable completeness fraction

    Returns:
        SourceCountsResult with completeness metrics
    """
...
def run_full_validation(
    ...
    min_snr: float = 5.0,
    match_radius_arcsec: float = 10.0,
) -> Tuple[Optional[AstrometryResult], Optional[FluxScaleResult], Optional[SourceCountsResult]]:
    ...
    Args:
        image_path: Path to FITS image
        catalog: Reference catalog ("nvss", "first", "vlass", "racs")
        validation_types: List of validation types to run
            Options: ["astrometry", "flux_scale", "source_counts"]
            Default: all types
        generate_html: Whether to generate HTML report
        html_output_path: Path for HTML report (required if generate_html=True)
        min_snr: Minimum SNR for source extraction
        match_radius_arcsec: Cross-match radius in arcseconds

    Returns:
        Tuple of (AstrometryResult, FluxScaleResult, SourceCountsResult)
        Results are None if that validation type was not requested
    ...
    astrometry_result: Optional[AstrometryResult] = None
    flux_scale_result: Optional[FluxScaleResult] = None
    source_counts_result: Optional[SourceCountsResult] = None
...
def _query_catalog_for_image(
    ...
    Args:
        image_path: Path to FITS image
        catalog: Catalog name ("nvss", "first", "vlass", "racs")
...
def _generate_html_report(
    output_path: str,
    image_path: Path,
    astrometry_result: Optional[AstrometryResult],
    flux_scale_result: Optional[FluxScaleResult],
    source_counts_result: Optional[SourceCountsResult],
) -> None:
    ...
    Args:
        ...
        astrometry_result: Astrometry validation results
        flux_scale_result: Flux scale validation results
        source_counts_result: Source counts validation results
    """
