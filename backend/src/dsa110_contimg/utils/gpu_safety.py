...
def check_system_memory_available(
    ...
    projected_used_pct = (
        (mem["used_gb"] + required_with_safety) / mem["total_gb"] * 100
    )
...
def setup_gpu_memory_pool(
    ...
    try:
        ...
        with cp.cuda.Device(gpu_id):
            ...
            logger.info(
                f"GPU {gpu_id} memory pool limit set to {limit_gb:.1f} GB"
            )
            return True
...
def check_visibility_allocation_safe(
    ...
    """
    required_gb = estimate_visibility_memory_gb(
        n_antennas, n_channels, n_times, n_pols
    )
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            if required_gpu_gb is not None:
                is_safe, reason = check_gpu_memory_available(
                    required_gpu_gb, gpu_id
                )
                if not is_safe:
                    if config.raise_on_violation:
...
def initialize_gpu_safety(
    ...
    if result["gpu_available"]:
        result["gpu_memory"] = [
            get_gpu_memory_info(i) for i in range(result["gpu_count"])
        ]
