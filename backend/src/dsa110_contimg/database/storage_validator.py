...
def validate_hdf5_storage(
    ...
    result.is_synchronized = (
        len(result.on_disk_not_in_db) == 0 and len(result.in_db_not_on_disk) == 0
    )
...
def get_storage_metrics(db_path: str, storage_dir: str) -> dict:
    ...
    metrics["count_matches"] = metrics["files_on_disk"] == metrics["files_in_db_stored"]
...
def normalize_group_timestamp(
    timestamp_iso: str, tolerance_seconds: int = GROUP_TIMESTAMP_TOLERANCE_SECONDS
) -> str:
    """
...
def full_reconciliation(
    ...
    if index_orphaned:
        index_result = index_orphaned_files(
            db_path, storage_dir, orphaned_files=pre_validation.on_disk_not_in_db, dry_run=dry_run
        )
