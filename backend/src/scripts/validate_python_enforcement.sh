...
if python3 -c "import dsa110_contimg; print('✅ PASS')" 2>&1 | grep -q "PASS"; then
    echo "   ✅ PASS: Package imports successfully with casa6"
else
    echo "   ❌ FAIL: Package import failed with casa6"
    exit 1
fi
...
if /usr/bin/python3 -c "import sys; sys.path.insert(0, '$SRC_DIR'); import dsa110_contimg" 2>&1 | grep -q "CRITICAL ERROR"; then
    echo "   ✅ PASS: System Python correctly rejected"
else
    echo "   ⚠️  WARNING: System Python was not rejected (may be expected if guard not loaded)"
fi
...
if python3 "$SRC_DIR/create_10min_mosaic.py" --help 2>&1 | head -1 | grep -q "usage"; then
    echo "   ✅ PASS: Entry point script works with casa6"
else
    echo "   ❌ FAIL: Entry point script failed"
    exit 1
fi
...
for ep in "${entry_points[@]}"; do
    if [ -f "$SRC_DIR/$ep" ]; then
        ...
        if echo "$shebang" | grep -q "/opt/miniforge/envs/casa6/bin/python"; then
            echo "   ✅ $ep: Correct shebang"
        else
            echo "   ❌ $ep: Incorrect shebang: $shebang"
            all_correct=false
        fi
    fi
done
...
if [ "$all_correct" = true ]; then
    echo "   ✅ PASS: All entry points have correct shebang"
else
    echo "   ❌ FAIL: Some entry points have incorrect shebang"
    exit 1
fi
