...
### Stage 1: File Ingestion :white_heavy_check_mark:
...
### Stage 2: MS Conversion :white_heavy_check_mark:
...
### Stage 3: Group Formation :white_heavy_check_mark:
...
### Stage 4: Calibration Solving :white_heavy_check_mark:
...
### Stage 5: Calibration Application :white_heavy_check_mark:

- Idempotent application
- Database state checked first
- **Status**: All icebergs fixed

### Stage 6: Imaging :white_heavy_check_mark:
...
### Stage 7: Mosaic Creation :white_heavy_check_mark:
...
### Stage 8: Registration & Publishing :white_heavy_check_mark:
...
- **Recovery**: Mosaics with warnings can be manually published via API after
  review
- **Status**: :white_heavy_check_mark: Working as designed (comment clarified)
...
- **Status**: :white_heavy_check_mark: Acceptable risk (database constraint provides protection)
...
- **Status**: :white_heavy_check_mark: Acceptable (atomic operation prevents partial state)
...
- **Status**: :white_heavy_check_mark: Acceptable (manual recovery available, errors logged)
...
- **Protection**: If registration fails, metadata is not stored (correct
  behavior)
- **Status**: :white_heavy_check_mark: Safe (metadata only stored if registration succeeds)
...
3. **Mosaic Finalized** :white_heavy_check_mark:
...
4. **Auto-Publish Triggered** :white_heavy_check_mark:
...
5. **Publish Complete** :white_heavy_check_mark:
...
- **Status**: :white_heavy_check_mark: Handled correctly
...
- **Protection**: Database UNIQUE constraint on `data_id` prevents duplicate
  registration
- **Status**: :white_heavy_check_mark: Protected (database constraint + status check)
...
- **Status**: :white_heavy_check_mark: Validated correctly
...
- **Status**: :white_heavy_check_mark: Handled (non-fatal, logged)

---

## Database Consistency Checks

### Transaction Safety :white_heavy_check_mark:
...
### State Transitions :white_heavy_check_mark:
...
### Unique Constraints :white_heavy_check_mark:
...
### All Critical Icebergs Fixed :white_heavy_check_mark:

1. :white_heavy_check_mark: Mosaic registration in `data_registry`
2. :white_heavy_check_mark: Mosaic finalization with QA/validation status
3. :white_heavy_check_mark: Auto-publish triggering
4. :white_heavy_check_mark: File move to `/data/`
5. :white_heavy_check_mark: Path validation
6. :white_heavy_check_mark: File move verification
7. :white_heavy_check_mark: Database rollback on failure
8. :white_heavy_check_mark: Metadata storage

### Remaining Non-Critical Considerations :white_heavy_check_mark:

1. :white_heavy_check_mark: Warnings prevent auto-publish (intentional)
2. :white_heavy_check_mark: Concurrent access protected (database constraints)
3. :white_heavy_check_mark: Error recovery available (manual publish API)
4. :white_heavy_check_mark: Registration retry safe (atomic operations)

### Workflow Completeness :white_heavy_check_mark:
