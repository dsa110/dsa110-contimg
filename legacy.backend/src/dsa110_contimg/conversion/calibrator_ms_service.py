...
class CalibratorMSGenerator:
        ...
        *,
        ...
        if hdf5_db is not None:
            ...
            if hdf5_db_env:
                ...
                # Try multiple candidate locations (similar to catalog resolution)
                products_db_path = Path(products_db)

                # Candidate paths in order of preference:
                candidates = [
                    # 1. Same directory as products_db (if products_db is absolute)
                    (
                        products_db_path.parent / "hdf5.sqlite3"
                        if products_db_path.is_absolute()
                        else None
                    ),
                    ...
                    (
                        ...
                        if os.getenv("PIPELINE_STATE_DIR")
                        else None
                    ),
                    # 4. Relative to products_db (fallback)
                    products_db_path.parent / "hdf5.sqlite3",
                ]

                # Use first candidate that exists, or first candidate if none exist
                self.hdf5_db = None
                for candidate in candidates:
                    if candidate is not None and candidate.exists():
                        self.hdf5_db = candidate
                        break
    ...
    def _find_best_candidate_group(
        self, groups: List["SubbandGroupInfo"], transit: Time
    ) -> Optional[Tuple[float, List[Optional[str]], Time]]:
        """Find the group whose mid-time is closest to transit.

        Args:
            groups: List of SubbandGroupInfo objects
            transit: Transit time

        Returns:
            Tuple of (delta_minutes, best_group_files, mid_time) or None if no valid candidates
        """
        from dsa110_contimg.database.hdf5_index import SubbandGroupInfo

        candidates = []
        for group_info in groups:
            # Get first non-None file to extract timestamp
            first_file = next((f for f in group_info.files if f is not None), None)
            if not first_file:
                continue
            ...
            try:
                ...
                candidates.append((dt_min, group_info.files, mid))
            except Exception:
                continue
    ...
    def _is_complete_subband_group(
        self, group: List[Optional[str]]
    ) -> Optional["SubbandGroupInfo"]:
        """Check if group has 12-16 subbands (missing 4 or fewer) and return metadata.

        NEW PROTOCOL: Groups with 12-16 subbands are accepted. Missing subbands
        will be filled with zero-padded synthetic subbands.

        Args:
            group: List of 16 file paths (may contain None for missing subbands)

        Returns:
            SubbandGroupInfo if group is acceptable (12-16 subbands), None otherwise
        """
        from dsa110_contimg.database.hdf5_index import SubbandGroupInfo
        ...
        try:
            group_info = SubbandGroupInfo.from_file_list(group)
            # Accept groups with 12-16 subbands
            if group_info.present_count >= 12 and group_info.present_count <= 16:
                return group_info
        except ValueError:
            pass
    ...
    def _log_found_groups(
        ...
        complete_count = sum(1 for g in groups if g.is_complete)
        semi_complete_count = sum(1 for g in groups if not g.is_complete)
        total_files = sum(g.present_count for g in groups)

        if complete_count > 0 and semi_complete_count > 0:
            logger.info(
                f"Found {len(groups)} group(s) ({complete_count} complete, {semi_complete_count} semi-complete) "
                f"with {total_files} total subbands for transit {transit.isot}"
            )
        elif complete_count > 0:
            logger.info(
                f"Found {complete_count} complete 16-subband group(s) ({total_files} total files) "
                f"for transit {transit.isot}"
            )
        else:
            logger.info(
                f"Found {semi_complete_count} semi-complete group(s) ({total_files} total subbands) "
                f"for transit {transit.isot}"
            )
    ...
    def _process_single_transit(
        ...
        # NEW PROTOCOL: Check if group is acceptable (12-16 subbands)
        group_info = self._is_complete_subband_group(gbest)
        if not group_info:
            present_count = len([f for f in gbest if f is not None])
            logger.info(
                f"Skipping transit {t.isot}: found group with {present_count} subbands "
                f"(need 12-16), best candidate: {os.path.basename(gbest[0]) if gbest[0] else 'N/A'}"
            )
            return None

        # Log group status
        if group_info.is_complete:
            logger.debug(f"Transit {t.isot}: Complete group (16/16 subbands)")
        else:
            logger.info(
                f"Transit {t.isot}: Semi-complete group ({group_info.present_count}/16 subbands), "
                f"missing: {sorted(group_info.missing_subband_codes)}"
            )

        # Fill missing subbands with synthetic zero-padded ones if needed
        import tempfile

        from dsa110_contimg.conversion.strategies.hdf5_orchestrator import fill_missing_subbands

        # Create temporary directory for synthetic subbands
        temp_dir = tempfile.mkdtemp(prefix="synthetic_subbands_")
        try:
            gbest_complete, temp_files = fill_missing_subbands(group_info, scratch_dir=temp_dir)

            # Sort the complete group
            ...
            gbest_sorted = sorted(gbest_complete, key=sort_by_subband, reverse=True)

            if temp_files:
                logger.info(
                    f"Filled {len(temp_files)} missing subband(s) ({sorted(group_info.missing_subband_codes)}) "
                    f"with synthetic zero-padded files for transit {t.isot}"
                )
        except Exception as e:
            logger.error(f"Failed to fill missing subbands for transit {t.isot}: {e}")
        ...
        # Use first file from complete sorted group for validation
        # (gbest_sorted now has all 16 files, including synthetic ones)
        reference_file = gbest_sorted[0] if gbest_sorted else None
        if not reference_file:
            logger.error(f"Transit {t.isot}: No files available in group")
            import shutil
        ...
        try:
            ...
            _, pt_dec_rad, _ = _peek_uvh5_phase_and_midtime(reference_file)
        ...
        if pt_dec_deg is not None:
            if not self._check_declination_match(
                ...
                logger.warning(
                    f"Transit {t.isot}: Declination mismatch for group {os.path.basename(reference_file)}, "
        ...
        pb_validation = self._validate_primary_beam(
            reference_file, ra_deg, dec_deg, min_pb_response, freq_ghz
        )
        if not pb_validation:
            metrics = self._calculate_primary_beam_metrics(
                reference_file, ra_deg, dec_deg, freq_ghz
            )
        ...
        pb_metrics = self._calculate_primary_beam_metrics(reference_file, ra_deg, dec_deg, freq_ghz)
        ...
        self._log_primary_beam_success(t, gbest_sorted, dt_min, pb_validation, ra_deg, dec_deg)

        result = self._build_transit_result_dict(
            calibrator_name,
            t,
            t0,
            t1,
            gbest_sorted,  # Use complete sorted group
            mid,
            dt_min,
            gbest_sorted,
            pb_validation,
        ...
        # Store temp_dir in result for cleanup later
    ...
    def _calculate_search_window(
        self, max_days_back: int, reference_time: Optional[Time] = None
    ) -> Tuple[str, str, Time]:
        """Calculate time range for HDF5 index query.

        Args:
            max_days_back: Maximum days to search back from reference_time
            reference_time: Reference time for search window (defaults to Time.now())

        Returns:
            Tuple of (start_time_str, end_time_str, cutoff_time)
        """
        ref_time = reference_time if reference_time is not None else Time.now()
        # Search from (reference - max_days_back) to (reference + max_days_back)
        # to center the search window around the reference time
        half_window = max_days_back / 2.0
        start_time = (
            (ref_time - half_window * u.day).to_datetime().strftime("%Y-%m-%d %H:%M:%S")
        )  # pylint: disable=no-member
        end_time = (
            (ref_time + half_window * u.day).to_datetime().strftime("%Y-%m-%d %H:%M:%S")
        )  # pylint: disable=no-member
        cutoff_time = ref_time - half_window * u.day  # pylint: disable=no-member
    ...
    def _query_hdf5_groups_in_window(
        ...
        Returns:
            List of SubbandGroupInfo objects
        """
        from dsa110_contimg.database.hdf5_index import SubbandGroupInfo
    ...
    def _find_matching_group_for_transit(
        ...
        Returns:
            Transit info dict if match found, None otherwise
        """
        for group_info_obj in groups:
            # Extract file list from SubbandGroupInfo object
            group_files = [f for f in group_info_obj.files if f is not None]
            if not group_files:
                continue
    ...
    def list_available_transits(
        ...
        *,
        ...
        Args:
            ...
            reference_time: Reference time for search window (defaults to Time.now()).
                           When provided, searches for transits around this time ±max_days_back/2.
        ...
        """
        # CRITICAL: When reference_time is provided (user-specified transit), skip stored transits
        # because the stored group_id values may be stale (calculated relative to a different time).
        # Force on-demand calculation to query fresh HDF5 groups around the user's specified time.
        if reference_time is None:
            # Try to use stored transit times from database (only when searching from Time.now())
            try:
                from dsa110_contimg.conversion.transit_precalc import (
                    get_calibrator_transits,
                )

                # Use reference_time if provided, otherwise default to Time.now()
                ref_time = Time.now()

                # Search for transits in window centered on reference_time
                half_window = max_days_back / 2.0
                min_transit_mjd = ref_time.mjd - half_window
                max_transit_mjd = ref_time.mjd + half_window
                ...
                stored_transits = get_calibrator_transits(
                    ...
                    max_transit_mjd=max_transit_mjd,
                    has_data=True,
                )
            ...
            logger.info(
                f"User-specified reference time ({reference_time.isot}); "
                f"skipping stored transits and calculating on-demand with fresh HDF5 group query."
            )

        # Fall back to on-demand calculation
        ra_deg, dec_deg = self._load_radec(calibrator_name)
        start_time, end_time, cutoff_time = self._calculate_search_window(
            max_days_back, reference_time=reference_time
        )
        ...
        # CRITICAL: Use reference_time (if provided) to calculate transits, not Time.now()
        # This ensures we find transits around the user-specified time, not backwards from now
        ref_time_for_transits = reference_time if reference_time is not None else Time.now()
        transits = previous_transits(ra_deg, start_time=ref_time_for_transits, n=max_days_back)
