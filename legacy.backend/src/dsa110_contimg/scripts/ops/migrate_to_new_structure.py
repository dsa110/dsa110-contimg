    ...
    if legacy_ms.exists():
        ...
        if science_dir.exists():
            for date_dir in science_dir.iterdir():
                if date_dir.is_dir():
                    science_base = new["raw_ms"] / "science" / date_dir.name
                    for ms_file in date_dir.iterdir():
                        if ms_file.is_dir() and ms_file.name.endswith(".ms"):
                            new_path = science_base / ms_file.name
        ...
        if calibrators_dir.exists():
            for date_dir in calibrators_dir.iterdir():
                if date_dir.is_dir():
                    calibrator_base = new["raw_ms"] / "calibrators" / date_dir.name
                    for ms_file in date_dir.iterdir():
                        if ms_file.is_dir() and ms_file.name.endswith(".ms"):
                            new_path = calibrator_base / ms_file.name
                            analysis["ms_files"].append(
                                {
                                    "old": ms_file,
                                    "new": new_path,
...
def main():
    ...
    if args.dry_run:
        ...
        print("4. Remove legacy symlinks after verification (Phase 4)")
