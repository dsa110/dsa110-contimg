...
def create_synthetic_catalog_db(
    ...
    Args:
        ...
        source_positions: List of (ra_deg, dec_deg, flux_jy) tuples for each source
        catalog_type: Catalog type ("nvss", "first", "rax", etc.)
        dec_strip: Declination strip (auto-detected from sources if None)
        add_noise: Add small random noise to positions/fluxes (realistic catalog errors)
        position_noise_arcsec: Position noise in arcseconds (default: 0.1")
        flux_noise_fraction: Flux noise as fraction of flux (default: 0.05 = 5%)
...
def create_synthetic_catalog_from_uvh5(
    ...
    Args:
        uvh5_path: Path to synthetic UVH5 file
        catalog_output_path: Path to output catalog database
        catalog_type: Catalog type ("nvss", "first", etc.)
        add_noise: Add realistic catalog errors
