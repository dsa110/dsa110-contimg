    ...
    for catalog_type in ["nvss", "first", "rax", "atnf"]:
        ...
        if show_database_status and dec_deg is not None:
            if not within_coverage:
                ...
                status_text = " :check_mark: DB exists"
            else:
                status_text = " :ballot_x: DB missing"
...
def plot_coverage_summary_table(
    ...
    for catalog_type in ["nvss", "first", "rax", "atnf"]:
        ...
        if dec_deg is None:
            ...
            if within_coverage == "Yes":
                ...
                status = ":check_mark: Ready" if exists else ":ballot_x: Missing"
            else:
                db_exists = "N/A"
    ...
    for i in range(len(table_data)):
        ...
        if ":check_mark:" in table_data[i][4]:
            status_cell.set_facecolor("#90EE90")  # Light green
        elif ":ballot_x:" in table_data[i][4]:
...
def main():
    ...
    if args.plot_type in ["both", "coverage"]:
        ...
        print(f":check_mark: Coverage plot: {plot_path}")
    ...
    if args.plot_type in ["both", "table"]:
        ...
        print(f":check_mark: Coverage table: {table_path}")
